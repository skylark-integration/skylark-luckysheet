define([
    ...
    './sheetmanage',
    '../global/tooltip'
], function (luckysheetConfigsetting, luckysheetFreezen, m_refresh, Store, locale, sheetmanage, tooltip) {
    'use strict';
    const {luckysheetrefreshgrid} = m_refresh;
    let gridW = 0, gridH = 0;
    return function luckysheetsizeauto(isRefreshCanvas = true) {
        if (!luckysheetConfigsetting.showinfobar) {
            Store.infobarHeight = 0;
            $('#luckysheet_info_detail').hide();
        } else {
            $('#luckysheet_info_detail').show();    // Store.infobarHeight = 56;
        ...
        //     Store.sheetBarHeight = 31;
        // }
        customSheetbarConfig();    // if (!luckysheetConfigsetting.showstatisticBar) {
