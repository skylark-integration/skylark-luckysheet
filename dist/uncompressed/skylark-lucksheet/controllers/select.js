define([
    ...
    '../global/method',
    '../locale/locale'
], function (menuButton, formula, m_dynamicArray, m_location, browser, dataVerificationCtrl, m_get, Store, method, locale) {
    'use strict';
        ...
        if (Store.luckysheet_select_save.length > 0) {
            for (let i = 0; i < Store.luckysheet_select_save.length; i++) {
                ...
                if (i == Store.luckysheet_select_save.length - 1) {
                    ...
                    //左上角选择区域框
                    formula.fucntionboxshow(rf, cf);    //focus单元格数据验证
                    //focus单元格数据验证
    ...
    function selectTitlesShow(rangeArr, isRestore = false) {
        ...
        for (let i = 0; i < s.length; i++) {
            let r1 = s[i]['row'][0], r2 = s[i]['row'][1], c1 = s[i]['column'][0], c2 = s[i]['column'][1];    // if(isRestore){
