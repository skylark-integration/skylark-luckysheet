define([
    ...
    './menuButton',
    '../global/method'
], function (a, cleargridelement, b, c, luckysheetcreatedom, tooltip, formula, d, rhchInit, editor, e, f, g, server, luckysheetConfigsetting, pivotTable, luckysheetsizeauto, luckysheetPostil, imageCtrl, dataVerificationCtrl, hyperlinkCtrl, luckysheetFreezen, h, i, Store, locale, j, k, l, menuButton, method) {
    'use strict';
    const sheetmanage = {
        ...
        addNewSheet: function (e, isPivotTable) {
            if (a.isEditMode() || Store.allowEdit === false) {
                return;
            }
            let _this = this;
            ...
            $('#luckysheet-sheet-container-c').append(f.replaceHtml(g.sheetHTML, {
                'index': index,
                'active': '',
                'name': sheetname,
                'style': '',
        ...
        createSheet: function () {
            ...
            for (let i = 0; i < Store.luckysheetfile.length; i++) {
                ...
                if (Store.currentSheetIndex == sheetIndex) {
                    btn.push(f.replaceHtml(g.sheetHTML, {
                        'index': sheetIndex,
        ...
        buildGridData: function (file) {
            let row = file.row == null ? Store.defaultrowNum : file.row, column = file.column == null ? Store.defaultcolumnNum : file.column, data = file.data && file.data.length > 0 ? file.data : b.datagridgrowth([], row, column), celldata = file.celldata;
            if (file.data && file.data.length > 0) {
                for (let i = 0; i < data.length; i++) {
                    for (let j = 0; j < data[0].length; j++) {
                        c.setcellvalue(i, j, data, data[i][j]);
                    }
                }
            } else {
                if (celldata && celldata.length > 0) {
                    for (let i = 0; i < celldata.length; i++) {
                        ...
                        let v = item.v;
                        if (r >= data.length) {
                            data = b.datagridgrowth(data, r - data.length + 1, 0);
                        }
                        if (c >= data[0].length) {
                            data = b.datagridgrowth(data, 0, c - data[0].length + 1);
                        }
                        c.setcellvalue(r, c, data, v);
                    }
                }
            }
        ...
        addGridData: function (celldata, row, column) {
            let data = b.datagridgrowth([], row, column);
            if (celldata != null) {
                for (let i = 0; i < celldata.length; i++) {
                    let item = celldata[i];
                    let r = item.r;
        ...
        initialjfFile: function (menu, title) {
            ...
            _this.nulldata = b.datagridgrowth([], Store.defaultrowNum, Store.defaultcolumnNum);
            ...
            menuButton.fontInitial(Store.fontList);
            file.data = data;
            ...
            setTimeout(function () {
                ...
                let ini = function () {
                    ...
                    let execF = function () {
                        ...
                        f.arrayRemoveItem(Store.asyncLoad, 'core');
        ...
        setSheetParam: function (isload = true) {
            ...
            hyperlinkCtrl.init();
            h.createFilterOptions(file['filter_select'], file['filter']);
        },
        restoreselect: function () {
            let index = this.getSheetIndex(Store.currentSheetIndex);
            let file = Store.luckysheetfile[index];
            i.selectHightlightShow(true);
            i.selectionCopyShow();
            if (file['scrollLeft'] != null && file['scrollLeft'] > 0) {
                $('#luckysheet-scrollbar-x').scrollLeft(file['scrollLeft']);
            } else {
        ...
        changeSheet: function (index, isPivotInitial, isNewSheet) {
            ...
            if (load != null) {
                ...
                if (loadSheetUrl == '' || Store.luckysheetcurrentisPivotTable || !!isNewSheet) {
                    ...
                    $('#luckysheet-grid-window-1').append(g.luckysheetlodingHTML());
            ...
            j.renderChartShow(index);
            luckysheetFreezen.initialFreezen(index);
            _this.restoreselect();
        },
        checkLoadSheetIndexToDataIndex: {},
        checkLoadSheetIndex: function (file) {
            let calchain = formula.getAllFunctionGroup();
            let chart = file.chart;
            let pivotTable = file.pivotTable;
            let ret = [], cache = {};
            ...
            if (calchain != null) {
                ...
                for (let i = 0; i < calchain.length; i++) {
                    ...
                    let formulaTxt = b.getcellFormula(f.r, f.c, dataindex);
                    if (formulaTxt == null) {
                        let file = Store.luckysheetfile[this.getSheetIndex(dataindex)];
                        file.data = this.buildGridData(file);
                        formulaTxt = b.getcellFormula(f.r, f.c, dataindex);
                        if (formulaTxt == null) {
                            continue;
                        }
                    }
        ...
        showSheet: function () {
            ...
            $('#luckysheet-cols-h-cells_0').css('width', Store.ch_width);
            ...
            l.zoomNumberDomBind(Store.zoomRatio);
        },
        ...
        getRangetxt: function (sheetIndex, range, currentIndex) {
            ...
            if (row0 == null && row1 == null) {
                return sheettxt + f.chatatABC(column0) + ':' + f.chatatABC(column1);
            } else if (column0 == null && column1 == null) {
                return sheettxt + (row0 + 1) + ':' + (row1 + 1);
            } else {
                if (column0 == column1 && row0 == row1) {
                    return sheettxt + f.chatatABC(column0) + (row0 + 1);
                } else {
                    return sheettxt + f.chatatABC(column0) + (row0 + 1) + ':' + f.chatatABC(column1) + (row1 + 1);
                }
            }
        },
        ...
        restoreFilter: function (sheetIndex) {
            let index = this.getSheetIndex(sheetIndex);
            let file = Store.luckysheetfile[index];
            if (f.getObjType(file.filter_select) == 'string') {
            ...
            h.createFilterOptions(file.filter_select);
            if (f.getObjType(file.filter) != 'object') {
                if (file.filter != null && f.getObjType(file.filter) == 'string') {
            ...
            $('#luckysheet-filter-options-sheet' + sheetIndex + ' .luckysheet-filter-options').each(function (i) {
                ...
                if (f.getObjType(item) != 'object') {
                    item = JSON.parse(item);
                }
                h.labelFilterOptionState($top, item.optionstate, item.rowhidden, item.caljs, false, item.st_r, item.ed_r, item.cindex, item.st_c, item.ed_c);
            ...
            d.jfrefreshgrid_rhcw(Store.flowdata.length, Store.flowdata[0].length, false);
        },
        ...
        execCache: function (item) {
            ...
            if (type == 'v') {
                ...
                if (s && f.getObjType(value) != 'object') {
                ...
                mst = index;
                med = index + len - 1;
                e.luckysheetdeletetable(mtype, mst, med, true);
            } else if (type == 'arc') {
                let rc = item.rc, index = value.index, len = value.len;
                let celldata = file.celldata;
                if (rc == 'r') {
                ...
                e.luckysheetextendtable(mtype, index, len, true);
            } else if (type == 'na') {
