define([
    ...
    '../store',
    '../locale/locale'
], function (a, server, luckysheetsizeauto, b, c, d, e, f, Store, locale) {
    'use strict';
    const imageCtrl = {
        imgItem: {
            type: '3',
        ...
        colorSelectDialog: function (currenColor) {
            ...
            $('body').append(f.replaceHtml(b.modelHTML, {
        ...
        init: function () {
            ...
            $('#luckysheet-image-showBoxs').off('mousedown.active').on('mousedown.active', '.luckysheet-modal-dialog-image', function (e) {
                if (!c.checkProtectionAuthorityNormal(Store.currentSheetIndex, 'editObjects', false)) {
                    return;
                }
                $(this).hide();
                let id = $(this).attr('id');
            ...
            $('#luckysheet-modal-dialog-activeImage').off('mousedown.move').on('mousedown.move', '.luckysheet-modal-dialog-content', function (e) {
                ...
                e.setluckysheet_scroll_status(true);
                e.stopPropagation();
            });
            $('#luckysheet-modal-dialog-activeImage').off('mousedown.resize').on('mousedown.resize', '.luckysheet-modal-dialog-resize-item', function (e) {
                ...
                let mouse = a.mouseposition(e.pageX, e.pageY);
        ...
        ref: function () {
            let _this = this;
            let file = Store.luckysheetfile[d.getSheetIndex(Store.currentSheetIndex)];
