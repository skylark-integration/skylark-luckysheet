define([
    ...
    '../locale/locale',
    'numeral'
], function (a, b, c, d, e, f, g, h, tooltip, editor, cleargridelement, luckysheetArray, analysis, i, j, k, sheetmanage, luckysheetsizeauto, server, l, Store, locale, numeral) {
    'use strict';
    const pivotTable = {
        pivotDatas: null,
        pivotSheetIndex: 0,
        ...
        getCellData: function (cursheetindex, datasheetindex, data_select_save) {
            ...
            let realIndex = a.getSheetIndex(sheetIndex);
            if (b.getObjType(Store.luckysheetfile[realIndex].pivotTable) != 'object') {
            ...
            let pivotrealIndex = a.getSheetIndex(_this.pivotDataSheetIndex);
            ...
            _this.origindata = c.getdatabyselectionD(otherfile.data, _this.pivot_select_save);
            let rowhidden = {};
            ...
            for (let c = 0; c < _this.celldata[1].length; c++) {
                let type = d.isdatatype(_this.celldata[1][c]);
        ...
        luckysheetsliderlistitemfilter: function ($filter) {
            ...
            if (rowhidden == null || rowhidden == '') {
                rowhidden = {};
            } else if (b.getObjType(rowhidden) == 'string') {
            ...
            setTimeout(function () {
                ...
                for (let r = 1; r < data.length; r++) {
                    ...
                    if (cell != null && cell.ct != null && cell.ct.t == 'd') {
                        let v = e.update('YYYY-MM-DD', cell.v);
                        let y = v.split('-')[0];
                        let m = v.split('-')[1];
                        let d = v.split('-')[2];
                        if (!(y in dvmap)) {
                        ...
                        let v, m;
                        if (cell == null || f.isRealNull(cell.v)) {
                            v = null;
                            m = null;
                        } else {
                            v = cell.v;
                ...
                if (JSON.stringify(vmap).length > 2) {
                    ...
                    vmapKeys = g.orderbydata1D(vmapKeys, true);
            ...
            b.showrightclickmenu($menu, toffset.left - 250, toffset.top);
        },
        ...
        setDatatojsfile: function (attr, value, cindex) {
            let _this = this;
            let index = a.getSheetIndex(_this.pivotSheetIndex);
        ...
        createPivotTable: function (e) {
            if (f.isEditMode() || Store.allowEdit === false) {
                return;
            }
            let _this = this;
        ...
        changePivotTable: function (index) {
            ...
            const _locale = locale();
            const locale_pivotTable = _locale.pivotTable;
            let pivotDataSheetIndex = Store.luckysheetfile[a.getSheetIndex(index)].pivotTable.pivotDataSheetIndex;
            let real_pivotDataSheetIndex = a.getSheetIndex(pivotDataSheetIndex);
        ...
        refreshPivotTable: function (isRefreshCanvas = true) {
            ...
            if (ret.length == 0) {
                ...
                data = c.datagridgrowth(d, addr + 20, addc + 10, true);
                for (let r = 0; r < rlen; r++) {
                    let x = [].concat(data[r]);
                    for (let c = 0; c < clen; c++) {
                        let value = '';
                        if (ret[r] != null && ret[r][c] != null) {
                            value = c.getcellvalue(r, c, ret);
                        }
                        x[c] = value;
                    }
                    data[r] = x;
            ...
            if (addr > 0 || addc > 0) {
                h.jfrefreshgridall(data[0].length, data.length, data, null, Store.luckysheet_select_save, 'datachangeAll', undefined, undefined, isRefreshCanvas);
            } else {
                h.jfrefreshgrid(data, Store.luckysheet_select_save, {}, null, isRefreshCanvas);
                i.selectHightlightShow();
            }
        ...
        pivotclick: function (row_index, col_index, index) {
            ...
            let file = Store.luckysheetfile[a.getSheetIndex(index)];
            ...
            if (isRangeClick && slider.is(':hidden')) {
                if (!l.checkProtectionAuthorityNormal(index, 'usePivotTablereports', false)) {
                    return;
                }
                slider.show();
                luckysheetsizeauto();
        ...
        getPivotTableData: function (dataindex) {
            ...
            let index = a.getSheetIndex(dataindex);
            let pivotTable = Store.luckysheetfile[index]['pivotTable'];
            if (b.getObjType(pivotTable) == 'object') {
        ...
        initialPivotManage: function (restore) {
            ...
            if (_this.initial) {
                _this.initial = false;
                $('body').append(k.luckysheetPivotTableHTML());
                $('#luckysheet-modal-dialog-slider-close').click(function () {
                    $('#luckysheet-modal-dialog-slider-pivot').hide();
                    luckysheetsizeauto();
                });
                $('body').append(b.replaceHtml(k.modelHTML, {
                ...
                $('body').append(b.replaceHtml(k.filtermenuHTML(), { 'menuid': 'pivotTableFilter' }));
                $('body').append(b.replaceHtml(k.filtersubmenuHTML(), { 'menuid': 'pivotTableFilter' }));
                $('body').append(k.pivottableconfigHTML());
                $('body').append(k.pivottablesumHTML());
                ...
                $('#luckysheet-modal-dialog-config-row, #luckysheet-modal-dialog-config-column').on('click', '.luckysheet-modal-dialog-slider-config-item-icon', function (e) {
                    ...
                    b.mouseclickposition($('#luckysheet-pivotTable-config-option'), toffset.left + $item.outerWidth(), toffset.top - 13, 'rightbottom');
                    e.stopPropagation();
                    return false;
                });
                ...
                $('#luckysheet-modal-dialog-config-value').on('click', '.luckysheet-modal-dialog-slider-config-item-icon', function (e) {
                    ...
                    $menu.data('item', $item);
                    b.mouseclickposition($menu, toffset.left + $item.outerWidth(), toffset.top - 13, 'rightbottom');
                    e.stopPropagation();
                    return false;
                });
                    ...
                    if ($('#luckysheet-pivotTableFilter-bycondition').next().is(':visible') && $('#luckysheet-pivotTableFilter-byvalue').next().is(':hidden') && $('#luckysheet-pivotTableFilter-selected span').data('value') != 'null') {
                        ...
                        for (let r = 1; r < d.length; r++) {
                            ...
                            if (value == 'cellnull') {
                                ...
                                let value1 = e.genarate(caljs['value1'])[2];
                                if (cell == null || f.isRealNull(cell.v)) {
                                    rowhidden[r] = 0;
                        ...
                        for (let r = 1; r < d.length; r++) {
                            ...
                            if (cell == null || f.isRealNull(cell.v)) {
                                ...
                                let fmt = e.update('YYYY-MM-DD', cell.v);
                ...
                $('#luckysheet-pivotTable-selection-confirm').click(function () {
                    ...
                    if ($.trim(val).length == 0 || $.trim(val).toUpperCase() == _this.jgridCurrentPivotInput.toUpperCase()) {
                        ...
                        if (val1.length > 1) {
                            ...
                            let index = a.getSheetIndex(Store.currentSheetIndex);
                            sheettxt = Store.luckysheetfile[index].name;
                            sheetIndex = Store.luckysheetfile[index].index;
                            rangetxt = val1[0];
                        }
                        if (Store.luckysheetfile[a.getSheetIndex(sheetIndex)].isPivotTable) {
                            if (f.isEditMode()) {
                        ...
                        col[0] = b.ABCatNum(rangetxt[0].replace(/[^A-Za-z]/g, ''));
                        col[1] = b.ABCatNum(rangetxt[1].replace(/[^A-Za-z]/g, ''));
                        if (col[0] > col[1]) {
                            if (f.isEditMode()) {
                                alert(locale_pivotTable.errorSelectionRange);
                            } else {
                ...
                $('#luckysheet-modal-dialog-config-filter, #luckysheet-modal-dialog-config-row, #luckysheet-modal-dialog-config-column, #luckysheet-modal-dialog-config-value').mousemove(function (e) {
                    if (_this.movestate) {
                        ...
                        let row_index = j.luckysheet_searcharray(position, curtop);
                        if (row_index == -1) {
            ...
            for (let i = 0; i < _this.celldata[0].length; i++) {
                ...
                if (!!_this.celldata[0][i] && !!_this.celldata[0][i]['m']) {
                    ...
                    name = c.getcellvalue(0, i, _this.celldata);
                }
                if (name != null) {
                    name = name.toString();
                }
            ...
            $('#luckysheet-dialog-pivotTable-range').html(a.getRangetxt(_this.pivotDataSheetIndex, _this.pivot_select_save));
            $('#luckysheet-modal-dialog-slider-pivot').show();
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                for (let x = 0; x <= i; x++) {
                    if (!!data[x] && !!data[x]['m']) {
                        ...
                        name += c.getcellvalue(x, null, data);
                    }
                }
                ret.push(name);
            }
        ...
        getnameArray: function (data, field) {
            ...
            for (let i = 0; i < field.length; i++) {
                ...
                if (!!data[field[i].index] && !!data[field[i].index]['m']) {
                    ...
                    c_value = c.getcellvalue(field[i].index, null, data);
                }
                ret.push(c_value);
            }
            return ret;
        ...
        orderbygroupchildren: function (childrens, orderby, order, dataposition) {
            ...
            let a = function (x, y) {
                ...
                if (orderby == 'self' || orderby == null) {
                    ...
                    if (d.isdatetime(f) && d.isdatetime(s)) {
                        return d.diff(f, s);
                    }
                } else {
        ...
        addStatisticsData: function (dataposition, valueobj, indicator, d_value) {
            ...
            if (d.isdatatypemulti(d_value)['num'] === true) {
                let num = b.numFormat(d_value);
        ...
        dataHandler: function (column, row, values, showType, celldata) {
            ...
            for (let i = 1; i < data.length; i++) {
                ...
                let v_str = '';
                for (let v = 0; v < values.length; v++) {
                    let d_value = c.getcellvalue(values[v].index, null, d);
            ...
            for (let indicator in dataposition) {
                ...
                if (json.sumtype == 'SUM') {
                    ...
                    json.result = b.numFormat(json.sum / json.count);
                ...
                let newAcc = b.numfloatlen(json.result);
                if (newAcc > json.acc) {
                    json.acc = newAcc;
                }
                json.result = b.numFormat(json.result, json.acc);
            }
                ...
                for (let c = 0; c < colLen; c++) {
                    ...
                    if (r < rowOver && c < colOver) {
                        ...
                        if (datacoltitle[r] != null) {
                            if (b.getObjType(datacoltitle[r][c - colOver]) == 'object') {
        ...
        drillDown: function (row_index, col_index) {
            if (!l.checkProtectionAuthorityNormal(Store.currentSheetIndex, 'usePivotTablereports')) {
                return;
            }
            let _this = this;
            ...
            for (let j = 1; j < _this.celldata.length; j++) {
                ...
                for (let x in obj) {
                    ...
                    if (!!_this.celldata[j][x] && !!_this.celldata[j][x]['m']) {
                        ...
                        value = c.getcellvalue(j, x, _this.celldata);
                    }
                    if (value != null) {
            ...
            for (let r = 0; r < rowArr.length; r++) {
                for (let c = 0; c < _this.celldata[0].length; c++) {
                    ...
                    if (!!_this.celldata[rowArr[r]][c] && !!_this.celldata[rowArr[r]][c]['m']) {
                        ...
                        value = c.getcellvalue(rowArr[r], c, _this.celldata);
                    }
                    if (value != null) {
                        value = value.toString();
                    } else {
