define([
    ...
    '../store',
    'dayjs'
], function (a, b, formula, c, tooltip, d, e, f, g, server, h, sheetmanage, locale, i, Store, dayjs) {
    'use strict';
    const conditionformat = {
        ...
        init: function () {
            ...
            $(document).off('click.CFadministerRuleConfirm').on('click.CFadministerRuleConfirm', '#luckysheet-administerRule-dialog-confirm', function () {
                if (!i.checkProtectionFormatCells(Store.currentSheetIndex)) {
                    return;
                }
                ...
                for (let c = 0; c < fileClone.length; c++) {
                    ...
                    Store.luckysheetfile[a.getSheetIndex(sheetIndex)]['luckysheet_conditionformat_save'] = fileClone[a.getSheetIndex(sheetIndex)]['luckysheet_conditionformat_save'];
                }
            ...
            $(document).off('click.CFadministerRuleFa').on('click.CFadministerRuleFa', '#luckysheet-administerRule-dialog .item .fa-table', function () {
                ...
                h.selectionCopyShow(_this.selectRange);
            });
            $(document).off('click.CFmultiRangeConfirm').on('click.CFmultiRangeConfirm', '#luckysheet-multiRange-dialog-confirm', function () {
                ...
                _this.fileClone[a.getSheetIndex(sheetIndex)]['luckysheet_conditionformat_save'][dataItem].cellrange = _this.getRangeByTxt(v);
                $('#luckysheet-modal-dialog-mask').show();
                $('#luckysheet-administerRule-dialog').show();
                let range = [];
                h.selectionCopyShow(range);
            });
            ...
            $(document).off('click.CFnewConditionRule').on('click.CFnewConditionRule', '#newConditionRule', function () {
                let sheetIndex = $('#luckysheet-administerRule-dialog .chooseSheet option:selected').val();
                if (!i.checkProtectionFormatCells(sheetIndex)) {
                    return;
                }
            ...
            $(document).off('click.CFnewConditionRuleConfirm').on('click.CFnewConditionRuleConfirm', '#luckysheet-newConditionRule-dialog-confirm', function () {
                        ...
                        if (type1 == 'number') {
                            ...
                            if (type2 == 'betweenness') {
                                ...
                                if (rangeArr1.length > 1) {
                                    ...
                                    if (r1 == r2 && c1 == c2) {
                                        v1 = e.getcellvalue(r1, c1, Store.flowdata);
                                        conditionRange.push({
                                            'row': rangeArr1[0].row,
                                ...
                                if (rangeArr2.length > 1) {
                                    ...
                                    if (r1 == r2 && c1 == c2) {
                                        v2 = e.getcellvalue(r1, c1, Store.flowdata);
                                        conditionRange.push({
                                            'row': rangeArr2[0].row,
                                ...
                                if (rangeArr.length > 1) {
                                    ...
                                    if (r1 == r2 && c1 == c2) {
                                        v = e.getcellvalue(r1, c1, Store.flowdata);
                                        conditionRange.push({
                                            'row': rangeArr[0].row,
                ...
                if (source == 0) {
                    ...
                    let fileH = $.extend(true, [], Store.luckysheetfile);
                    let historyRules = _this.getHistoryRules(fileH);
                    let ruleArr = Store.luckysheetfile[a.getSheetIndex(Store.currentSheetIndex)]['luckysheet_conditionformat_save'] == undefined ? [] : Store.luckysheetfile[a.getSheetIndex(Store.currentSheetIndex)]['luckysheet_conditionformat_save'];
                    ruleArr.push(rule);
                    Store.luckysheetfile[a.getSheetIndex(Store.currentSheetIndex)]['luckysheet_conditionformat_save'] = ruleArr;
                    let fileC = $.extend(true, [], Store.luckysheetfile);
                    let currentRules = _this.getCurrentRules(fileC);
                    _this.ref(historyRules, currentRules);
                    if (server.allowUpdate) {
                    ...
                    let ruleArr = !!_this.fileClone[a.getSheetIndex(Store.currentSheetIndex)]['luckysheet_conditionformat_save'] ? _this.fileClone[a.getSheetIndex(Store.currentSheetIndex)]['luckysheet_conditionformat_save'] : [];
                    ruleArr.push(rule);
                    _this.fileClone[a.getSheetIndex(Store.currentSheetIndex)]['luckysheet_conditionformat_save'] = ruleArr;
            ...
            $(document).off('click.CFeditorConditionRule').on('click.CFeditorConditionRule', '#editorConditionRule', function () {
                ...
                let rule = {
                    ...
                    'itemIndex': itemIndex,
                    'data': _this.fileClone[a.getSheetIndex(sheetIndex)]['luckysheet_conditionformat_save'][itemIndex]
                };
            ...
            $(document).off('click.CFeditorConditionRuleConfirm').on('click.CFeditorConditionRuleConfirm', '#luckysheet-editorConditionRule-dialog-confirm', function () {
                ...
                _this.fileClone[a.getSheetIndex(sheetIndex)]['luckysheet_conditionformat_save'][itemIndex] = rule;
            ...
            $(document).off('click.CFdeleteConditionRule').on('click.CFdeleteConditionRule', '#deleteConditionRule', function () {
                ...
                let itemIndex = $('#luckysheet-administerRule-dialog .ruleList .listBox .item.on').attr('data-item');
                _this.fileClone[a.getSheetIndex(sheetIndex)]['luckysheet_conditionformat_save'].splice(itemIndex, 1);
                _this.administerRuleDialog();
            });
            ...
            $(document).on('click', '.range .fa-table', function () {
                ...
                h.selectionCopyShow(_this.getRangeByTxt(v));
            });
        ...
        singleRangeDialog: function (source, value) {
            ...
            const conditionformat_Text = locale().conditionformat;
            $('body').append(b.replaceHtml(g.modelHTML, {
        ...
        multiRangeDialog: function (dataItem, value) {
            ...
            h.selectionCopyShow(_this.getRangeByTxt(value));
        },
        getTxtByRange: function (range) {
            if (range.length > 0) {
                ...
                for (let s = 0; s < range.length; s++) {
                    let r1 = range[s].row[0], r2 = range[s].row[1];
                    let c1 = range[s].column[0], c2 = range[s].column[1];
                    txt.push(a.getRangetxt(Store.currentSheetIndex, {
                        'row': [
                            r1,
                            r2
                        ],
        ...
        getConditionRuleList: function (index) {
            ...
            let ruleArr = _this.fileClone[a.getSheetIndex(index)].luckysheet_conditionformat_save;
            if (ruleArr != null && ruleArr.length > 0) {
                ...
                for (let i = 0; i < ruleArr.length; i++) {
                    ...
                    for (let s = 0; s < cellrange.length; s++) {
                        ...
                        rangeTxtArr.push(b.chatatABC(c1) + (r1 + 1) + ':' + b.chatatABC(c2) + (r2 + 1));
                    }
                ...
                $('#luckysheet-administerRule-dialog .ruleList .listBox .item canvas').each(function (i) {
                    ...
                    if (type == 'dataBar') {
                        ...
                        let h2 = 46 * 32 / w1;
                        if (l == '0') {
                            can.drawImage(g.luckysheet_CFiconsImg, 0, t * 32, w1, h1, 0, (18 - h2) / 2, w2, h2);
                        } else if (l == '5') {
                            can.drawImage(g.luckysheet_CFiconsImg, 210, t * 32, w1, h1, 0, (18 - h2) / 2, w2, h2);
                        }
                    }
                });
        ...
        getConditionRuleName: function (conditionName, conditionRange, conditionValue) {
            ...
            if (conditionRange[0] != null) {
                v = b.chatatABC(conditionRange[0]['column'][0]) + (conditionRange[0]['row'][0] + 1);
            } else {
                v = conditionValue[0];
            }
            const conditionformat_Text = locale().conditionformat;
            if (conditionName == 'greaterThan') {
                ...
                if (conditionRange[1] != null) {
                    v2 = b.chatatABC(conditionRange[1]['column'][0]) + (conditionRange[1]['row'][0] + 1);
                } else {
                    v2 = conditionValue[1];
                }
            ...
            if (ruleType == 'dataBar' || ruleType == 'colorGradation' || ruleType == 'icons') {
                ...
                if (type1 == 'number') {
                    ...
                    if (rule.conditionRange[0] != null) {
                        val1 = a.getRangetxt(Store.currentSheetIndex, {
                    ...
                    if (conditionName == 'betweenness') {
                        ...
                        if (rule.conditionRange[1] != null) {
                            val2 = a.getRangetxt(Store.currentSheetIndex, {
        ...
        getCFPartRange: function (sheetIndex, range1, range2) {
            let ruleArr = [];
            let cf = Store.luckysheetfile[a.getSheetIndex(sheetIndex)].luckysheet_conditionformat_save;
                        ...
                        for (let s = 0; s < cellrange.length; s++) {
                            for (let r = cellrange[s].row[0]; r <= cellrange[s].row[1]; r++) {
                                for (let c = cellrange[s].column[0]; c <= cellrange[s].column[1]; c++) {
                                    ...
                                    let cell = d[r][c];
                                    if (b.getObjType(cell) == 'object' && cell['ct'] != null && cell['ct'].t == 'n' && cell.v != null) {
                            ...
                            if (conditionName == 'greaterThan' || conditionName == 'lessThan' || conditionName == 'equal' || conditionName == 'textContains') {
                                for (let r = cellrange[s].row[0]; r <= cellrange[s].row[1]; r++) {
                                    for (let c = cellrange[s].column[0]; c <= cellrange[s].column[1]; c++) {
                                        ...
                                        let cell = d[r][c];
                                        if (b.getObjType(cell) != 'object' || c.isRealNull(cell.v)) {
                                            continue;
                                        }
                                ...
                                if (conditionValue0.toString().indexOf('-') == -1) {
                                    dBig = f.genarate(conditionValue0)[2];
                                    dSmall = f.genarate(conditionValue0)[2];
                                } else {
                                    let str = conditionValue0.toString().split('-');
                                    dBig = f.genarate(str[1].trim())[2];
                                    dSmall = f.genarate(str[0].trim())[2];
                                }
                                for (let r = cellrange[s].row[0]; r <= cellrange[s].row[1]; r++) {
                                    for (let c = cellrange[s].column[0]; c <= cellrange[s].column[1]; c++) {
                                        if (d[r] == null || d[r][c] == null) {
                                        ...
                                        if (d[r][c].ct != null && d[r][c].ct.t == 'd') {
                                            let cellVal = e.getcellvalue(r, c, d);
                                ...
                                for (let r = cellrange[s].row[0]; r <= cellrange[s].row[1]; r++) {
                                    for (let c = cellrange[s].column[0]; c <= cellrange[s].column[1]; c++) {
                                        ...
                                        if (d[r][c].ct != null && d[r][c].ct.t == 'n') {
                                            dArr.push(e.getcellvalue(r, c, d));
                                        }
                                    }
                                }
