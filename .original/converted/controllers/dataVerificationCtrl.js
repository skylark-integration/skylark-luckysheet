define([
    ...
    '../locale/locale',
    '../store'
], function (a, formula, b, c, d, tooltip, e, f, editor, g, h, server, menuButton, sheetmanage, i, locale, Store) {
    'use strict';
    const dataVerificationCtrl = {
        ...
        createDialog: function () {
            ...
            $('body').append(a.replaceHtml(g.modelHTML, {
            ...
            $(document).off('click.dvSaveConfirm').on('click.dvSaveConfirm', '#luckysheet-dataVerification-dialog-confirm', function (e) {
                ...
                if (type == 'dropdown') {
                    ...
                    if (type2 == 'bw' || type2 == 'nb') {
                        ...
                        if (!c.isdatetime(value2)) {
                            tooltip.info('<i class="fa fa-exclamation-triangle"></i>', dvText.tooltipInfo6);
                            return;
                        }
                        if (c.diff(value1, value2) > 0) {
                ...
                for (let r = str; r <= edr; r++) {
                    for (let c = stc; c <= edc; c++) {
                        ...
                        if (type == 'checkbox') {
                            e.setcellvalue(r, c, d, item.value2);
                        }
                    }
                }
                if (type == 'checkbox') {
        ...
        cellFocus: function (r, c, clickMode) {
            ...
            let cellValue = f.getcellvalue(r, c, null);
            if (b.isRealNull(cellValue)) {
                return;
            }
        ...
        validateCellData: function (cellValue, item) {
            ...
            if (type == 'dropdown') {
                ...
                if (!c.isdatetime(cellValue)) {
                    return false;
                }
                if (type2 == 'bw' && (c.diff(cellValue, value1) < 0 || c.diff(cellValue, value2) > 0)) {
                    return false;
                }
                if (type2 == 'nb' && (c.diff(cellValue, value1) >= 0 && c.diff(cellValue, value2) <= 0)) {
                    return false;
                }
                if (type2 == 'eq' && c.diff(cellValue, value1) != 0) {
                    return false;
                }
                if (type2 == 'ne' && c.diff(cellValue, value1) == 0) {
                    return false;
                }
                if (type2 == 'bf' && c.diff(cellValue, value1) >= 0) {
                    return false;
                }
                if (type2 == 'nbf' && c.diff(cellValue, value1) < 0) {
                    return false;
                }
                if (type2 == 'af' && c.diff(cellValue, value1) <= 0) {
                    return false;
                }
                if (type2 == 'naf' && c.diff(cellValue, value1) > 0) {
                    return false;
                }
            } else if (type == 'validity') {
