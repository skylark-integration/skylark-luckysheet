define([
    ...
    '../global/method',
    '../global/api'
], function (sheetmanage, server, pivotTable, conditionformat, luckysheetPostil, imageCtrl, dataVerificationCtrl, hyperlinkCtrl, a, b, formula, json, cleargridelement, c, d, Store, e, method, f) {
    'use strict';
    ...
    const controlHistory = {
        redo: function (e) {
            ...
            if (ctr.type == 'datachange') {
                ...
                c.jfrefreshgrid(ctr.data, ctr.range, allParam);
                ...
                c.jfrefreshgrid_pastcut(s, t, ctr.RowlChange);
            } else if (ctr.type == 'rangechange') {
                formulaHistoryHanddler(ctr);
                c.jfrefreshrange(ctr.data, ctr.range, ctr.cdformat);
            } else if (ctr.type == 'resize') {
                Store.config = ctr.config;
                Store.luckysheetfile[d.getSheetIndex(ctr.sheetIndex)].config = Store.config;
                ...
                Store.luckysheetfile[d.getSheetIndex(ctr.sheetIndex)].images = images;
                ...
                c.jfrefreshgridall(ctr.data[0].length, ctr.data.length, ctr.data, ctr.config, ctr.range, ctr.ctrlType, ctr.ctrlValue, ctr.cdformat);
            } else if (ctr.type == 'extend') {
                c.jfrefreshgridall(ctr.data[0].length, ctr.data.length, ctr.data, ctr.config, ctr.range, 'dele', ctr.ctrlValue);
            } else if (ctr.type == 'dele') {
                let ctrlValue1 = $.extend(true, {}, ctr.ctrlValue);
                ctrlValue1.restore = true;
                c.jfrefreshgridall(ctr.data[0].length, ctr.data.length, ctr.data, ctr.config, ctr.range, 'extend', ctrlValue1);
                ...
                c.jfrefreshgrid_adRC(ctr.data, ctr.config, 'delRC', ctrlValue, ctr.calc, ctr.filterObj, ctr.cf, ctr.af, ctr.freezen, ctr.dataVerification, ctr.hyperlink);
            } else if (ctr.type == 'delRC') {
                let ctrlValue = $.extend(true, {}, ctr.ctrlValue);
                ctrlValue.restore = true;
                ctrlValue.direction = 'lefttop';
                c.jfrefreshgrid_adRC(ctr.data, ctr.config, 'addRC', ctrlValue, ctr.calc, ctr.filterObj, ctr.cf, ctr.af, ctr.freezen, ctr.dataVerification, ctr.hyperlink);
            } else if (ctr.type == 'deleteCell') {
                c.jfrefreshgrid_deleteCell(ctr.data, ctr.config, ctr.ctrl, ctr.calc, ctr.filterObj, ctr.cf, ctr.dataVerification, ctr.hyperlink);
            } else if (ctr.type == 'showHidRows') {
                Store.config = ctr.config;
                Store.luckysheetfile[d.getSheetIndex(ctr.sheetIndex)].config = ctr.config;
                ...
                c.jfrefreshgridall(ctr.data[0].length, ctr.data.length, ctr.data, null, ctr.range, 'datachangeAll', ctr.ctrlValue);
            } else if (ctr.type == 'datachangeAll_filter_clear') {
                b.createFilterOptions(ctr.filter_save);
                $('#luckysheet-filter-options-sheet' + Store.currentSheetIndex + ' .luckysheet-filter-options').each(function (i) {
                    ...
                    b.labelFilterOptionState($top, item.optionstate, item.rowhidden, item.caljs, false, item.st_r, item.ed_r, item.cindex, item.st_c, item.ed_c);
                });
                ...
                Store.config = ctr.config;
                Store.luckysheetfile[d.getSheetIndex(Store.currentSheetIndex)].config = Store.config;
                ...
                b.labelFilterOptionState($top, json.hasKey(ctr.rowhidenPre), ctr.rowhidenPre, ctr.caljs, true, st_r, ed_r, cindex, st_c, ed_c);
                Store.config = ctr.config;
                Store.luckysheetfile[d.getSheetIndex(Store.currentSheetIndex)].config = Store.config;
                if (Store.config['rowhidden'] == null) {
                    Store.config['rowhidden'] = {};
                ...
                Store.luckysheetfile[d.getSheetIndex(ctr.sheetIndex)].pivotTable = ctr.pivotTable;
                ...
                Store.luckysheetfile[d.getSheetIndex(ctr.sheetIndex)].name = ctr.oldtxt;
                ...
                Store.luckysheetfile[d.getSheetIndex(ctr.sheetIndex)].color = ctr.oldcolor;
                ...
                for (let i = 0; i < historyRules.length; i++) {
                    ...
                    Store.luckysheetfile[d.getSheetIndex(sheetIndex)]['luckysheet_conditionformat_save'] = historyRules[i]['luckysheet_conditionformat_save'];
                    if (server.allowUpdate) {
        ...
        undo: function () {
            ...
            if (ctr.type == 'datachange') {
                ...
                c.jfrefreshgrid(ctr.curdata, ctr.range, allParam);
            } else if (ctr.type == 'pasteCut') {
                c.jfrefreshgrid_pastcut(ctr.source, ctr.target, ctr.RowlChange);
            } else if (ctr.type == 'rangechange') {
                formulaHistoryHanddler(ctr, 'undo');
                c.jfrefreshrange(ctr.curdata, ctr.range, ctr.curCdformat);
            } else if (ctr.type == 'resize') {
                ...
                c.jfrefreshgridall(ctr.curdata[0].length, ctr.curdata.length, ctr.curdata, ctr.curconfig, ctr.currange, ctr.ctrlType, ctr.ctrlValue, ctr.curCdformat);
            } else if (ctr.type == 'extend') {
                c.jfrefreshgridall(ctr.curdata[0].length, ctr.curdata.length, ctr.curdata, ctr.curconfig, ctr.currange, ctr.ctrlType, ctr.ctrlValue);
            } else if (ctr.type == 'dele') {
                let ctrlValue1 = $.extend(true, {}, ctr.ctrlValue);
                ctrlValue1.restore = true;
                c.jfrefreshgridall(ctr.curdata[0].length, ctr.curdata.length, ctr.curdata, ctr.curconfig, ctr.currange, ctr.ctrlType, ctr.ctrlValue);
            } else if (ctr.type == 'addRC') {
                c.jfrefreshgrid_adRC(ctr.curData, ctr.curConfig, 'addRC', ctr.ctrlValue, ctr.curCalc, ctr.curFilterObj, ctr.curCf, ctr.curAf, ctr.curFreezen, ctr.curDataVerification, ctr.curHyperlink);
            } else if (ctr.type == 'delRC') {
                c.jfrefreshgrid_adRC(ctr.curData, ctr.curConfig, 'delRC', ctr.ctrlValue, ctr.curCalc, ctr.curFilterObj, ctr.curCf, ctr.curAf, ctr.curFreezen, ctr.curDataVerification, ctr.curHyperlink);
            } else if (ctr.type == 'deleteCell') {
                c.jfrefreshgrid_deleteCell(ctr.curData, ctr.curConfig, ctr.ctrl, ctr.curCalc, ctr.curFilterObj, ctr.curCf, ctr.curDataVerification, ctr.curHyperlink);
            } else if (ctr.type == 'showHidRows') {
                Store.config = ctr.curconfig;
                Store.luckysheetfile[d.getSheetIndex(ctr.sheetIndex)].config = ctr.curconfig;
                ...
                formula.execFunctionGroup();
                c.jfrefreshgridall(ctr.curdata[0].length, ctr.curdata.length, ctr.curdata, null, ctr.currange, 'datachangeAll', ctr.ctrlValue);
            } else if (ctr.type == 'datachangeAll_filter_clear') {
                ...
                let $top = $('#luckysheet-filter-options-sheet' + Store.currentSheetIndex + ' .luckysheet-filter-options').eq(ctr['optionsindex']);
                let st_r = $top.data('str'), ed_r = $top.data('edr'), cindex = $top.data('cindex'), st_c = $top.data('stc'), ed_c = $top.data('edc');
                b.labelFilterOptionState($top, json.hasKey(ctr.rowhidden), ctr.rowhidden, ctr.caljs, true, st_r, ed_r, cindex, st_c, ed_c);
                Store.config = ctr.curconfig;
                Store.luckysheetfile[d.getSheetIndex(Store.currentSheetIndex)].config = Store.config;
                server.saveParam('cg', Store.currentSheetIndex, Store.config['rowhidden'], { 'k': 'rowhidden' });
                c.jfrefreshgrid_rhcw(Store.flowdata.length, Store.flowdata[0].length);
                ...
                Store.luckysheetfile[d.getSheetIndex(ctr.sheetIndex)].pivotTable = ctr.pivotTablecur;
                pivotTable.getCellData(ctr.sheetIndex);
                pivotTable.initialPivotManage(true);
                pivotTable.refreshPivotTable();
            } else if (ctr.type == 'addSheet') {
                ...
                Store.luckysheetfile[d.getSheetIndex(ctr.sheetIndex)].name = ctr.txt;
                ...
                Store.luckysheetfile[d.getSheetIndex(ctr.sheetIndex)].color = ctr.color;
                let luckysheetcurrentSheetitem = $('#luckysheet-sheets-item' + ctr.sheetIndex);
                luckysheetcurrentSheetitem.find('.luckysheet-sheets-item-color').remove();
                if (ctr.color != null) {
                ...
                for (let i = 0; i < currentRules.length; i++) {
                    ...
                    Store.luckysheetfile[d.getSheetIndex(sheetIndex)]['luckysheet_conditionformat_save'] = currentRules[i]['luckysheet_conditionformat_save'];
                    if (server.allowUpdate) {
