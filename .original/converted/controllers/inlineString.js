define([
    ...
    function updateInlineStringFormat(cell, attr, value, $input) {
        ...
        if ($textEditor.length > 0) {
            if (startContainer === endContainer) {
                ...
                b.selectTextContent($textEditor.find('span').get(seletedNodeIndex));
            } else {
                if (startContainer.parentNode.tagName == 'SPAN' && endContainer.parentNode.tagName == 'SPAN') {
                    ...
                    b.selectTextContentCross(spans.get(startSeletedNodeIndex), spans.get(endSeletedNodeIndex));
                }
            }
    ...
    function enterKeyControll(cell) {
        ...
        if ($textEditor.length > 0) {
            ...
            if (startContainer.parentNode.tagName == 'SPAN') {
                ...
                let cssText = a.getFontStyleByCell(cell);
                if (sright == '') {
            ...
            b.selectTextContentCollapse($textEditor.find('span').get(spanIndex), startOffset + 1);
        } else if ($functionbox.length > 0) {
        }
    }
    ...
    function convertCssToStyleList(cssText) {
        ...
        cssTextArray.forEach(s => {
            s = s.toLowerCase();
            let key = a.textTrim(s.substr(0, s.indexOf(':')));
            let value = a.textTrim(s.substr(s.indexOf(':') + 1));
            if (key == 'font-weight') {
                if (value == 'bold') {
                    styleList['bl'] = 1;
                } else {
    ...
    function getCssText(cssText, attr, value) {
        ...
        let s = a.getFontStyleByCell(styleObj, undefined, undefined, false);
        let ukey = a.textTrim(s.substr(0, s.indexOf(':')));
        let uvalue = a.textTrim(s.substr(s.indexOf(':') + 1));
    ...
    function extendCssText(origin, cover, isLimit = true) {
        ...
        for (let i = 0; i < originArray.length; i++) {
            ...
            so = so.toLowerCase();
            let okey = a.textTrim(so.substr(0, so.indexOf(':')));
            let ovalue = a.textTrim(so.substr(so.indexOf(':') + 1));
            if (isLimit) {
            ...
            for (let a = 0; a < coverArray.length; a++) {
                let sc = coverArray[a];
                sc = sc.toLowerCase();
                let ckey = a.textTrim(sc.substr(0, sc.indexOf(':')));
                let cvalue = a.textTrim(sc.substr(sc.indexOf(':') + 1));
                if (okey == ckey) {
                    newCss += ckey + ':' + cvalue + ';';
                    isAdd = false;
                    continue;
