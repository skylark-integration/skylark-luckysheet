define([
    ...
    '../global/json',
    '../global/format'
], function (a, editor, b, tooltip, c, d, e, server, locale, Store, menuButton, conditionformat, alternateformat, f, g, cleargridelement, h, i, json, j) {
    'use strict';
    ...
    function orderbydatafiler(str, stc, edr, edc, index, asc) {
        ...
        data = i.orderbydata(data, index - stc, asc);
        for (let r = str; r <= edr; r++) {
            for (let c = stc; c <= edc; c++) {
        ...
        if (Store.config['rowlen'] != null) {
            ...
            cfg = c.rowlenByRange(d, str, edr, cfg);
            allParam = {
                'cfg': cfg,
                'RowlChange': true
            };
    ...
    function createFilter() {
        if (!f.checkProtectionAuthorityNormal(Store.currentSheetIndex, 'filter')) {
            return;
        }
        ...
        if (Store.luckysheetfile[a.getSheetIndex(Store.currentSheetIndex)].isPivotTable) {
            return;
        }
        ...
        if (last['row'][0] == last['row'][1] && last['column'][0] == last['column'][1]) {
            ...
            for (let c = 0; c < Store.flowdata[curR].length; c++) {
                ...
                if (cell != null && !b.isRealNull(cell.v)) {
                    if (st_c == null) {
                        st_c = c;
                    }
                } else if (st_c != null) {
            ...
            e.luckysheetMoveEndCell('down', 'range');
    ...
    function initialFilterHandler() {
        ...
        $('#luckysheet-cell-main').on('click', '.luckysheet-filter-options', function (e) {
            ...
            setTimeout(function () {
                ...
                for (let r = st_r + 1; r <= ed_r; r++) {
                    ...
                    if (cell != null && !b.isRealNull(cell.v) && cell.ct != null && cell.ct.t == 'd') {
                        let v = j.update('YYYY-MM-DD', cell.v);
                        let y = v.split('-')[0];
                        let m = v.split('-')[1];
                        let d = v.split('-')[2];
                        if (!(y in dvmap)) {
                ...
                if (JSON.stringify(vmap).length > 2) {
                    ...
                    vmapKeys = i.orderbydata1D(vmapKeys, true);
            ...
            g.showrightclickmenu($menu, toffset.left, toffset.top + 20);
            e.stopPropagation();
            return false;
        });
        ...
        $(document).off('click.orderbyColorConfirm').on('click.orderbyColorConfirm', '#luckysheet-filter-orderby-color-submenu #luckysheet-filter-orderby-color-confirm', function () {
            ...
            for (let r = st_r + 1; r <= ed_r; r++) {
                ...
                if (bg_filter && fc_filter) {
                    if (!(bg in bg_colorMap) && (!(fc in fc_colorMap) || cell == null || b.isRealNull(cell.v))) {
                        rowhidden[r] = 0;
                    }
                } else if (bg_filter) {
                    ...
                    if (!(fc in fc_colorMap) || cell == null || b.isRealNull(cell.v)) {
                        rowhidden[r] = 0;
                    }
                }
            }
            ...
            Store.config = cfg;
            Store.luckysheetfile[a.getSheetIndex(Store.currentSheetIndex)].config = Store.config;
        ...
        $('#luckysheet-filter-initial').click(function () {
            ...
            $('#luckysheet-filter-selected-sheet' + Store.currentSheetIndex + ', #luckysheet-filter-options-sheet' + Store.currentSheetIndex).remove();
            $('#luckysheet-filter-menu, #luckysheet-filter-submenu').hide();
            Store.luckysheetfile[a.getSheetIndex(Store.currentSheetIndex)].filter = null;
            Store.luckysheetfile[a.getSheetIndex(Store.currentSheetIndex)].filter_select = null;
            ...
            if ($('#luckysheet-filter-bycondition').next().is(':visible') && $('#luckysheet-filter-byvalue').next().is(':hidden') && $('#luckysheet-filter-selected span').data('value') != 'null') {
                ...
                for (let r = st_r + 1; r <= ed_r; r++) {
                    ...
                    if (value == 'cellnull') {
                        ...
                        let value1 = j.genarate(caljs['value1'])[2];
                        if (cell == null || b.isRealNull(cell.v)) {
                            rowhidden[r] = 0;
                ...
                for (let r = st_r + 1; r <= ed_r; r++) {
                    ...
                    if (cell == null || b.isRealNull(cell.v)) {
                        ...
                        let fmt = j.update('YYYY-MM-DD', cell.v);
