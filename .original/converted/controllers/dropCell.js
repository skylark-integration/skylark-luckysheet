define([
    ...
    '../locale/locale',
    'dayjs'
], function (a, b, c, d, e, f, editor, formula, conditionformat, g, h, i, j, Store, locale, dayjs) {
    'use strict';
    const luckysheetDropCell = {
        ...
        createIcon: function () {
            ...
            let row = a.rowLocationByIndex(row_index)[1], row_pre = a.rowLocationByIndex(row_index)[0];
            let col = a.colLocationByIndex(col_index)[1], col_pre = a.colLocationByIndex(col_index)[0];
            ...
            $('#luckysheet-dropCell-icon').mouseover(function () {
                ...
                $('body').append(j.replaceHtml(_this.typeListHtml, {
            ...
            for (let r = str_r; r <= end_r; r++) {
                for (let c = str_c; c <= end_c; c++) {
                    if (!!Store.flowdata[r][c]) {
                        ...
                        if (j.getObjType(cell) == 'object' && cell['v'] != null && cell['f'] == null) {
        ...
        e.update: function () {
            let _this = this;
            if (!g.checkProtectionLockedRangeList([_this.applyRange], Store.currentSheetIndex)) {
                return;
            }
            ...
            let file = Store.luckysheetfile[i.getSheetIndex(Store.currentSheetIndex)];
            let cfg = $.extend(true, {}, Store.config);
            let borderInfoCompute = c.getBorderInfoCompute();
                        ...
                        for (let j = apply_str_r; j <= apply_end_r; j++) {
                            ...
                            if (cell.f != null) {
                                ...
                                if (cell.spl != null) {
                                    ...
                                    if (d.isRealNum(cell.v) && !/^\d{6}(18|19|20)?\d{2}(0[1-9]|1[12])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/i.test(cell.v)) {
                                        if (cell.v == Infinity || cell.v == -Infinity) {
                                            ...
                                            if (cell.v.toString().indexOf('e') > -1) {
                                                ...
                                                let mask = e.genarate(Math.round(cell.v * 1000000000) / 1000000000);
            ...
            f.jfrefreshgrid(d, Store.luckysheet_select_save, allParam);
            h.selectHightlightShow();
        },
        ...
        FillDays: function (data, len, step) {
            ...
            for (let i = 1; i <= len; i++) {
                let index = (i - 1) % data.length;
                let d = $.extend(true, {}, data[index]);
                let date = e.update('yyyy-MM-dd', d['v']);
                date = dayjs(date).add(step * i, 'days').format('YYYY-MM-DD');
                d['v'] = e.genarate(date)[2];
                d['m'] = e.update(d['ct']['fa'], d['v']);
                applyData.push(d);
            }
            return applyData;
        },
