    ...
    const func_methods = {
        getCellDataArr: function (rangeObj, nullCellType, isNeglectNullCell) {
            ...
            if (rangeObj.data == null) {
                ...
                if (a.getObjType(rangeObj.data) === 'array') {
                    ...
                    if (b.isRealNull(rangeObj.data.v)) {
                        if (!isNeglectNullCell) {
                            if (nullCellType == 'number') {
                                dataArr.push(0);
                            } else if (nullCellType == 'text') {
        ...
        getDataArr: function (arr, isNeglectNaN) {
            ...
            if (a.getObjType(arr[0]) == 'array') {
                for (let i = 0; i < arr.length; i++) {
                    for (let j = 0; j < arr[i].length; j++) {
                        if (isNeglectNaN && !b.isRealNum(arr[i][j])) {
                            continue;
                        }
                ...
                for (let i = 0; i < arr.length; i++) {
                    if (isNeglectNaN && !b.isRealNum(arr[i])) {
                        continue;
                    }
                    dataArr.push(arr[i]);
                }
        ...
        getFirstValue: function (data, nullCellType) {
            ...
            } else if (a.getObjType(data) == 'object' && data.startCell != null) {
                if (data.data == null) {
                    if (nullCellType == 'number') {
                        value = 0;
                    } else if (nullCellType == 'text') {
                    ...
                    if (data.rowl == 1 && data.coll == 1) {
                        value = data.data;
                        if (value == null || b.isRealNull(value.v)) {
                            if (nullCellType == 'number') {
                                value = 0;
                            } else if (nullCellType == 'text') {
                                value = '';
                        ...
                        if (data.data[0][0].mc != null && data.data[0][0].mc.rs == data.rowl && data.data[0][0].mc.cs == data.coll) {
                            ...
                            if (value == null || b.isRealNull(value.v) || value.mc != null) {
                                if (nullCellType == 'number') {
                                    value = 0;
                                } else if (nullCellType == 'text') {
                                    value = '';
        ...
        getCellBoolen: function (data) {
            ...
            if (b.valueIsError(cumulative)) {
                return cumulative;
            }
            if (a.getObjType(cumulative) == 'boolean') {
            } else if (a.getObjType(cumulative) == 'string' && (cumulative.toLowerCase() == 'true' || cumulative.toLowerCase() == 'false')) {
                if (cumulative.toLowerCase() == 'true') {
                    cumulative = true;
                } else if (cumulative.toLowerCase() == 'false') {
                    cumulative = false;
                }
            } else if (b.isRealNum(cumulative)) {
        ...
        getCellDate: function (data) {
            ...
            if (a.getObjType(data) == 'array') {
                ...
                if (data.data == null || a.getObjType(data.data) == 'array' || b.isRealNull(data.data.v)) {
                    return error.v;
                }
                date_text = data.data.v;
                if (data.data.ct != null && data.data.ct.t == 'd') {
                    date_text = c.update('YYYY-MM-DD h:mm:ss', date_text);
                }
            } else {
                date_text = data;
            }
        ...
        getCellrangeDate: function (data) {
            ...
            let date = [];
            if (a.getObjType(data) == 'array') {
                if (a.getObjType(data[0]) == 'array' && !_this.isDyadicArr(data)) {
                    return error.v;
                }
                ...
                if (data.data == null) {
                    ...
                    if (a.getObjType(data.data) == 'array') {
                        for (let i = 0; i < data.data.length; i++) {
                            for (let j = 0; j < data.data[i].length; j++) {
                                if (data.data[i][j] != null && !b.isRealNull(data.data[i][j].v)) {
                                    let value = data.data[i][j].v;
                                    if (data.data[i][j].ct != null && data.data[i][j].ct.t == 'd') {
                                        value = c.update('YYYY-MM-DD h:mm:ss', value);
                                    }
                                    date.push(value);
                                } else {
                                    date.push(0);
