define([
    ...
    '../methods/get',
    '../store'
], function (editor, formula, a, b, c, conditionformat, luckysheetFreezen, d, e, f, g, Store) {
    'use strict';
    function luckysheetextendtable(type, index, value, direction, sheetIndex) {
        sheetIndex = sheetIndex || Store.currentSheetIndex;
        if (type == 'row' && !f.checkProtectionAuthorityNormal(sheetIndex, 'insertRows')) {
            return;
        } else if (type == 'column' && !f.checkProtectionAuthorityNormal(sheetIndex, 'insertColumns')) {
            return;
        }
        let curOrder = g.getSheetIndex(sheetIndex);
        ...
        if (calcChain != null && calcChain.length > 0) {
            for (let i = 0; i < calcChain.length; i++) {
                ...
                let calc_r = calc.r, calc_c = calc.c, calc_i = calc.index, calc_funcStr = b.getcellFormula(calc_r, calc_c, calc_i);
                if (type == 'row') {
        ...
        if (file.index == Store.currentSheetIndex) {
            a.jfrefreshgrid_adRC(d, cfg, 'addRC', {
                'index': index,
                'len': value,
                'direction': direction,
                'rc': type1,
    ...
    function luckysheetextendData(rowlen, newData) {
        ...
        Store.luckysheetfile[g.getSheetIndex(Store.currentSheetIndex)].data = d;
        Store.config = cfg;
        Store.luckysheetfile[g.getSheetIndex(Store.currentSheetIndex)].config = Store.config;
        a.jfrefreshgrid_rhcw(Store.flowdata.length, Store.flowdata[0].length);
    }
    function luckysheetdeletetable(type, st, ed, sheetIndex) {
        sheetIndex = sheetIndex || Store.currentSheetIndex;
        if (type == 'row' && !f.checkProtectionAuthorityNormal(sheetIndex, 'deleteRows')) {
            return;
        } else if (type == 'column' && !f.checkProtectionAuthorityNormal(sheetIndex, 'deleteColumns')) {
            return;
        }
        let curOrder = g.getSheetIndex(sheetIndex);
        let file = Store.luckysheetfile[curOrder];
        ...
        if (luckysheetFreezen.freezenhorizontaldata != null && type == 'row') {
            let freezen_scrollTop = luckysheetFreezen.freezenhorizontaldata[2];
            let freezen_st = e.luckysheet_searcharray(Store.visibledatarow, freezen_scrollTop);
            if (freezen_st == -1) {
                freezen_st = 0;
            }
            let freezen_row_st = luckysheetFreezen.freezenhorizontaldata[1] - 1;
        ...
        if (luckysheetFreezen.freezenverticaldata != null && type == 'column') {
            let freezen_scrollLeft = luckysheetFreezen.freezenverticaldata[2];
            let freezen_st2 = e.luckysheet_searcharray(Store.visibledatacolumn, freezen_scrollLeft);
            if (freezen_st2 == -1) {
                freezen_st2 = 0;
            }
            let freezen_col_st = luckysheetFreezen.freezenverticaldata[1] - 1;
        ...
        if (file.index == Store.currentSheetIndex) {
            a.jfrefreshgrid_adRC(d, cfg, 'delRC', {
                'index': st,
                'len': ed - st + 1,
                'rc': type1
            }, newCalcChain, newFilterObj, newCFarr, newAFarr, newFreezen, newDataVerification, newHyperlink);
    ...
    function luckysheetDeleteCell(type, str, edr, stc, edc, sheetIndex) {
        sheetIndex = sheetIndex || Store.currentSheetIndex;
        if (!f.checkProtectionNotEnable(sheetIndex)) {
            return;
        }
        let curOrder = g.getSheetIndex(sheetIndex);
        let file = Store.luckysheetfile[curOrder];
        ...
        if (file.index == Store.currentSheetIndex) {
            a.jfrefreshgrid_deleteCell(d, cfg, {
                type: type,
                'str': str,
                'edr': edr,
                'stc': stc,
