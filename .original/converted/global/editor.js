define([
    ...
    '../methods/get',
    '../store'
], function (browser, formula, a, b, c, Store) {
    'use strict';
    const editor = {
        ...
        controlHandler: function (dataChe, range) {
            ...
            if (addr > 0 || addc > 0) {
                d = a.datagridgrowth([].concat(d), addr, addc, true);
            }
            ...
            if (addr > 0 || addc > 0) {
                b.jfrefreshgridall(d[0].length, d.length, d, null, Store.luckysheet_select_save, 'datachangeAll');
            } else {
                b.jfrefreshrange(d, Store.luckysheet_select_save);
            }
        },
        clearRangeByindex: function (st_r, ed_r, st_c, ed_c, sheetIndex) {
            let index = c.getSheetIndex(sheetIndex);
