define([
    ...
    '../locale/locale',
    '../store'
], function (a, b, server, formula, editor, c, sheetmanage, d, locale, Store) {
    'use strict';
    function getdatabyselection(range, sheetIndex) {
        ...
        if (sheetIndex != null && sheetIndex != Store.currentSheetIndex) {
            d = Store.luckysheetfile[b.getSheetIndex(sheetIndex)]['data'];
            cfg = Store.luckysheetfile[b.getSheetIndex(sheetIndex)]['config'];
        } else {
    ...
    function getdatabyselectionD(d, range) {
        ...
        let dynamicArray_compute = c.dynamicArrayCompute(Store.luckysheetfile[b.getSheetIndex(Store.currentSheetIndex)]['dynamicArray']);
        let data = [];
        if (d == null) {
            return data;
        }
    ...
    function getcellvalue(r, c, data, type) {
        ...
        let retv = d_value;
        if (a.getObjType(d_value) == 'object') {
            retv = d_value[type];
            if (type == 'f' && retv != null) {
    ...
    function checkstatusByCell(cell, a) {
        ...
        if (a in tf || a == 'fs' && d.isInlineStringCell(cell)) {
            if (foucsStatus == null) {
                foucsStatus = '0';
            } else {
            ...
            if (foucsStatus == null) {
                ...
                if (foucsStatus.indexOf('rgba') > -1) {
                    foucsStatus = a.rgbTohex(foucsStatus);
                }
            }
        } else if (a == 'bg') {
            if (foucsStatus == null) {
