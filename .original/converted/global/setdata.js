define([
    ...
    '../store/index'
], function (a, b, c, server, luckysheetConfigsetting, Store) {
    'use strict';
    function setcellvalue(r, c, d, v) {
        ...
        if (b.isRealNull(cell) || (a.getObjType(cell) === 'string' || a.getObjType(cell) === 'number') && cell === v) {
            cell = {};
        }
        if (vupdate.toString().substr(0, 1) == "'") {
            ...
            cell.v = false;
        } else if (b.valueIsError(vupdate)) {
            cell.m = vupdate.toString();
            if (cell.ct != null) {
                cell.ct.t = 'e';
            } else {
            ...
            if (cell.f != null && b.isRealNum(vupdate) && !/^\d{6}(18|19|20)?\d{2}(0[1-9]|1[12])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/i.test(vupdate)) {
                ...
                if (b.isRealNum(vupdate) && !/^\d{6}(18|19|20)?\d{2}(0[1-9]|1[12])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/i.test(vupdate)) {
                    vupdate = parseFloat(vupdate);
                    cell.v = parseFloat(vupdate);
                    cell.ct = {
                        'fa': 'General',
