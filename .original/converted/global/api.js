define([
...
], function (Store, a, b, locale, method, formula, func_methods, tooltip, json, editor, luckysheetformula, cleargridelement, c, d, e, f, g, h, i, j, k, l, m, server, menuButton, selection, luckysheetConfigsetting, luckysheetFreezen, luckysheetsizeauto, sheetmanage, conditionformat, n, o, p, q, controlHistory, r, dataVerificationCtrl, imageCtrl, dayjs, s) {
    'use strict';
    const IDCardReg = /^\d{6}(18|19|20)?\d{2}(0[1-9]|1[12])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/i;
    function getCellValue(row, column, options = {}) {
        if (!j.isRealNum(row) || !j.isRealNum(column)) {
            return tooltip.info('Arguments row or column cannot be null or undefined.', '');
        }
        let curSheetOrder = b.getSheetIndex(Store.currentSheetIndex);
        ...
        let return_v;
        if (a.getObjType(cellData) == 'object') {
    ...
    function setCellValue(row, column, value, options = {}) {
        ...
        let {order = b.getSheetIndex(Store.currentSheetIndex), isRefresh = true, success} = { ...options };
        ...
        if (value == null || value.toString().length == 0) {
            ...
            d.setcellvalue(row, column, data, value);
        } else if (value instanceof Object) {
            let curv = {};
            let cell = data[row][column];
            if (j.isRealNull(cell)) {
                cell = {};
            }
            if (value.f != null && value.v == null) {
                ...
                formula.delFunctionGroup(row, column);
                d.setcellvalue(row, column, data, curv);
            }
    ...
    function deleteCell(move, row, column, options = {}) {
        ...
        i.luckysheetDeleteCell(moveType, row, row, column, column, sheetIndex);
        if (success && typeof success === 'function') {
            success();
        }
    }
    ...
    function frozenFirstRow(order) {
        ...
        if (!order || order == b.getSheetIndex(Store.currentSheetIndex)) {
    ...
    function setBothFrozen(isRange, options = {}) {
        let curSheetOrder = b.getSheetIndex(Store.currentSheetIndex);
        let {range, order = curSheetOrder, success} = { ...options };
        let isCurrentSheet = !order || order == b.getSheetIndex(Store.currentSheetIndex);
        const locale_frozen = locale().freezen;
        cancelFrozen(order);
        if (!isRange) {
    ...
    function insertRowOrColumn(type, index = 0, options = {}) {
        ...
        i.luckysheetextendtable(type, index, number, 'lefttop', sheetIndex);
        if (success && typeof success === 'function') {
            success();
        }
    }
    ...
    function deleteRowOrColumn(type, startIndex, endIndex, options = {}) {
        if (!j.isRealNum(startIndex) || !j.isRealNum(endIndex)) {
        ...
        i.luckysheetdeletetable(type, startIndex, endIndex, sheetIndex);
        if (success && typeof success === 'function') {
            success();
        }
    }
    ...
    function setRowHeight(rowInfo, options = {}) {
        if (a.getObjType(rowInfo) != 'object') {
            return tooltip.info('The rowInfo parameter is invalid.', '');
        }
        let {order = b.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
    ...
    function setColumnWidth(columnInfo, options = {}) {
        if (a.getObjType(columnInfo) != 'object') {
    ...
    function getRowHeight(rowInfo, options = {}) {
        if (a.getObjType(rowInfo) != 'array' || rowInfo.length == 0) {
            return tooltip.info('The rowInfo parameter is invalid.', '');
        }
        let {order = b.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
    ...
    function getColumnWidth(columnInfo, options = {}) {
        if (a.getObjType(columnInfo) != 'array' || columnInfo.length == 0) {
            return tooltip.info('The columnInfo parameter is invalid.', '');
        }
        let {order = b.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
    ...
    function getRangeAxis() {
        ...
        rangeArr.forEach(ele => {
            let axisText = b.getRangetxt(sheetIndex, {
                column: ele.column,
                row: ele.row
            });
    ...
    function getRangeValue(options = {}) {
        let curOrder = b.getSheetIndex(Store.currentSheetIndex);
        let {range, order = curOrder} = { ...options };
        let file = Store.luckysheetfile[order];
        if (!range || typeof range === 'object') {
            return g.getdatabyselection(range, file.index);
        } else if (typeof range === 'string') {
            if (formula.iscelldata(range)) {
                return g.getdatabyselection(formula.getcellrange(range), file.index);
            } else {
    ...
    function getRangeHtml(options = {}) {
        let {range = Store.luckysheet_select_save, order = b.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        ...
        } else if (a.getObjType(range) == 'object') {
        ...
        if (cfg['merge'] != null) {
            ...
            for (let s = 0; s < range.length; s++) {
                ...
                has_PartMC = j.hasPartMC(cfg, r1, r2, c1, c2);
                if (has_PartMC) {
                    break;
                }
            }
            ...
            for (let j = 0; j < colIndexArr.length; j++) {
                ...
                if (d[r] != null && d[r][c] != null) {
                    ...
                    if (d[r][c].ct != null && d[r][c].ct.fa != null && d[r][c].ct.fa.match(reg)) {
                        c_value = g.getcellvalue(r, c, d);
                    } else {
                        c_value = g.getcellvalue(r, c, d, 'm');
                    }
    ...
    function getRangeArray(dimensional, options = {}) {
        ...
        let {range = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1], order = b.getSheetIndex(Store.currentSheetIndex)} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
        ...
        let cfg = $.extend(true, {}, file.config);
        if (cfg['merge'] != null) {
            let has_PartMC = j.hasPartMC(cfg, r1, r2, c1, c2);
            if (has_PartMC) {
                return tooltip.info('Cannot perform this operation on partially merged cells', '');
            }
        }
    ...
    function getRangeJson(isFirstRowTitle, options = {}) {
        ...
        if (config['merge'] != null) {
            ...
            has_PartMC = j.hasPartMC(config, r1, r2, c1, c2);
            if (has_PartMC) {
                if (j.isEditMode()) {
        ...
        let getdata = g.getdatabyselection(range, file.index);
        let arr = [];
        if (getdata.length === 0) {
            return;
        }
        if (isFirstRowTitle) {
            if (getdata.length === 1) {
                ...
                for (let i = 0; i < getdata[0].length; i++) {
                    obj[g.getcellvalue(0, i, getdata)] = '';
                }
                arr.push(obj);
            } else {
                for (let r = 1; r < getdata.length; r++) {
                    ...
                    for (let c = 0; c < getdata[0].length; c++) {
                        if (g.getcellvalue(0, c, getdata) == undefined) {
                            obj[''] = g.getcellvalue(r, c, getdata);
                        } else {
                            obj[g.getcellvalue(0, c, getdata)] = g.getcellvalue(r, c, getdata);
                        }
                    }
                    arr.push(obj);
                }
            ...
            for (let r = 0; r < getdata.length; r++) {
                let obj = {};
                for (let c = 0; c < getdata[0].length; c++) {
                    obj[a.chatatABC(c + st)] = g.getcellvalue(r, c, getdata);
                }
                arr.push(obj);
            }
        }
    ...
    function getRangeDiagonal(type, options = {}) {
        ...
        let getdata = g.getdatabyselection(range, order);
        let arr = [];
        if (getdata.length === 0) {
            return;
        }
    ...
    function setRangeShow(range, options = {}) {
        ...
        if (a.getObjType(range) == 'array') {
            for (let i = 0; i < range.length; i++) {
                if (a.getObjType(range[i]) === 'string') {
        ...
        let {show = true, order = b.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
    ...
    function setRangeFilter(type, options = {}) {
        ...
        let {range = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1], order = b.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
    ...
    function setRangeMerge(type, options = {}) {
        ...
        let curSheetOrder = b.getSheetIndex(Store.currentSheetIndex), curRange = Store.luckysheet_select_save;
        let {range = curRange, order = curSheetOrder, success} = { ...options };
        ...
        for (let i = 0; i < range.length; i++) {
            ...
            for (let r = r1; r <= r2; r++) {
                for (let c = c1; c <= c2; c++) {
                    let cell = data[r][c];
                    if (a.getObjType(cell) == 'object' && 'mc' in cell) {
                        isHasMc = true;
                        break;
                    }
                }
            ...
            for (let i = 0; i < range.length; i++) {
                ...
                if (type == 'all') {
                    ...
                    for (let r = r1; r <= r2; r++) {
                        for (let c = c1; c <= c2; c++) {
                            let cell = data[r][c];
                            if (cell != null && (!j.isRealNull(cell.v) || cell.f != null) && !isfirst) {
                                fv = $.extend(true, {}, cell);
                                isfirst = true;
                            }
                            data[r][c] = {
    ...
    function cancelRangeMerge(options = {}) {
        let curRange = Store.luckysheet_select_save, curSheetOrder = b.getSheetIndex(Store.currentSheetIndex);
        let {range = curRange, order = curSheetOrder, success} = { ...options };
        let file = Store.luckysheetfile[order], cfg = $.extend(true, {}, file.config), data = $.extend(true, [], file.data);
        if (data.length == 0) {
            data = $.extend(true, [], sheetmanage.buildGridData(file));
    ...
    function setRangeSort(type, options = {}) {
        ...
        let curSheetOrder = b.getSheetIndex(Store.currentSheetIndex), curRange = Store.luckysheet_select_save[0];
        let {range = curRange, order = curSheetOrder, success} = { ...options };
        ...
        data = e.orderbydata([].concat(data), 0, type === 'asc');
        for (let r = r1; r <= r2; r++) {
            for (let c = c1; c <= c2; c++) {
        ...
        let allParam = {};
        if (cfg['rowlen'] != null) {
            cfg = f.rowlenByRange(fileData, r1, r2, cfg);
            allParam = {
                'cfg': cfg,
                'RowlChange': true
            };
    ...
    function setRangeSortMulti(hasTitle, sort, options = {}) {
        ...
        sort.forEach(sortItem => {
            let i = sortItem.i;
            i -= c1;
            data = e.orderbydata([].concat(data), i, sortItem.sort === 'asc');
        });
        for (let r = str; r <= r2; r++) {
            for (let c = c1; c <= c2; c++) {
        ...
        let allParam = {};
        if (cfg['rowlen'] != null) {
            cfg = f.rowlenByRange(fileData, str, r2, cfg);
            allParam = {
                'cfg': cfg,
                'RowlChange': true
            };
    ...
    function setRangeConditionalFormatDefault(conditionName, conditionValue, options = {}) {
        ...
        if (a.getObjType(conditionValue) != 'array' || conditionValue.length == 0) {
            return tooltip.info('The conditionValue parameter is invalid.', '');
        }
        let {format = {
                'textColor': '#000000',
                'cellColor': '#ff0000'
            }, cellrange = Store.luckysheet_select_save, order = b.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        let data = file.data;
        if (data == null || data.length == 0) {
            data = sheetmanage.buildGridData(file);
        ...
        if (a.getObjType(cellrange) == 'string') {
            cellrange = conditionformat.getRangeByTxt(cellrange);
        } else if (a.getObjType(cellrange) == 'object') {
            cellrange = [cellrange];
        }
        if (a.getObjType(cellrange) != 'array') {
    ...
    function setRangeConditionalFormat(type, options = {}) {
        ...
        let {format, cellrange = Store.luckysheet_select_save, order = b.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('Incorrect worksheet index', '');
        }
        if (type == 'dataBar') {
            ...
            if (a.getObjType(format) != 'array' || format.length < 1 || format.length > 2) {
                return tooltip.info('The format parameter is invalid.', '');
            }
            ...
            if (a.getObjType(format) != 'array' || format.length < 2 || format.length > 3) {
                return tooltip.info('The format parameter is invalid.', '');
            }
        } else if (type == 'icons') {
            if (format == null) {
            ...
            if (a.getObjType(format) != 'string' || !formatValues.includes(format)) {
                return tooltip.info('The format parameter is invalid.', '');
            }
            switch (format) {
    ...
    function deleteRange(move, options = {}) {
        ...
        if (move == 'left') {
            i.luckysheetDeleteCell('moveLeft', str, edr, stc, edc, order);
        } else if (move == 'up') {
            i.luckysheetDeleteCell('moveUp', str, edr, stc, edc, order);
        }
        if (success && typeof success === 'function') {
            success();
        }
    ...
    function matrixOperation(type, options = {}) {
        ...
        let getdata = g.getdatabyselection(range);
        let arr = [];
        if (getdata.length === 0) {
            return;
        }
    ...
    function setSheetAdd(options = {}) {
        ...
        $('#luckysheet-sheet-container-c').append(a.replaceHtml(p.sheetHTML, {
            'index': index,
            'active': '',
            'name': sheetname,
            'style': '',
    ...
    function setSheetCopy(options = {}) {
        let {targetOrder, order = b.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
    ...
    function setSheetActive(order, options = {}) {
        if (order == null || !j.isRealNum(order) || Store.luckysheetfile[order] == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
        let file = Store.luckysheetfile[order];
        let {success} = { ...options };
    ...
    function setSheetName(name, options = {}) {
        if (a.getObjType(name) != 'string' || name.toString().length == 0) {
    ...
    function setSheetColor(color, options = {}) {
        if (a.getObjType(color) != 'string' || color.toString().length == 0) {
    ...
    function setSheetMove(type, options = {}) {
        if (type != 'left' && type != 'right' && !j.isRealNum(type)) {
    ...
    function setSheetZoom(zoom, options = {}) {
        if (!j.isRealNum(zoom) || zoom < 0.1 || zoom > 4) {
    ...
    function getScreenshot(options = {}) {
        ...
        let str = range.row[0], edr = range.row[1], stc = range.column[0], edc = range.column[1];
        let has_PartMC = j.hasPartMC(Store.config, str, edr, stc, edc);
        if (has_PartMC) {
            return tooltip.info('Cannot perform this operation on partially merged cells', '');
        }
    ...
    function getSheetData(options = {}) {
        let {
            order = b.getSheetIndex(Store.currentSheetIndex)
        } = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
    ...
    function getLuckysheetfile() {
        return b.getluckysheetfile();
    }
    function setDataVerification(optionItem, options = {}) {
        if (a.getObjType(optionItem) != 'object') {
        ...
        for (let r = str; r <= edr; r++) {
            for (let c = stc; c <= edc; c++) {
                ...
                if (type == 'checkbox') {
                    item.checked ? d.setcellvalue(r, c, data, item.value1) : d.setcellvalue(r, c, data, item.value2);
                }
            }
        }
        if (file.index == Store.currentSheetIndex) {
    ...
    function insertImage(src, options = {}) {
        let {order = b.getSheetIndex(Store.currentSheetIndex), rowIndex, colIndex, success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
    ...
    function deleteImage(options = {}) {
        let {order = b.getSheetIndex(Store.currentSheetIndex), idList = 'all', success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
        ...
        if (idList != 'all' && a.getObjType(idList) != 'array') {
            return tooltip.info('The idList parameter is invalid.', '');
        }
        if (a.getObjType(idList) == 'array') {
            idList.forEach(item => {
                delete images[item];
            });
        } else {
    ...
    function toJson() {
        ...
        b.getluckysheetfile().forEach((file, index) => {
            if (file.data == undefined) {
                return;
            }
            toJsonOptions.data[index].row = a.getObjType(file.data) === 'array' ? file.data.length : 0;
            toJsonOptions.data[index].column = a.getObjType(file.data[0]) === 'array' ? file.data[0].length : 0;
        });
