define([
    ...
    function getRangetxt(sheetIndex, range, currentIndex) {
        ...
        if (row0 == null && row1 == null) {
            return sheettxt + util.chatatABC(column0) + ':' + util.chatatABC(column1);
        } else if (column0 == null && column1 == null) {
            return sheettxt + (row0 + 1) + ':' + (row1 + 1);
        } else {
            if (column0 == column1 && row0 == row1) {
                return sheettxt + util.chatatABC(column0) + (row0 + 1);
            } else {
                return sheettxt + util.chatatABC(column0) + (row0 + 1) + ':' + util.chatatABC(column1) + (row1 + 1);
            }
        }
    }
