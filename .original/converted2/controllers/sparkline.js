define(['../store'], function (Store) {
    ...
    const luckysheetSparkline = {
        ...
        line: {
            ...
            getRegion: function (el, x, y) {
                let i, regionMap = this.regionMap;
        ...
        bullet: {
            ...
            render: function (el, userValues) {
                ...
                let vlen = this.values.length, target = this.target, i, shape;
                for (i = 2; i < vlen; i++) {
        ...
        pie: {
            ...
            render: function (el, userValues) {
                this.init(el, userValues);
                let target = this.target, values = this.values, options = this.options, radius = this.radius, borderWidth = options.get('borderWidth'), shape, i;
                if (borderWidth) {
        ...
        _drawShape: function (shapeid, path, lineColor, fillColor, lineWidth) {
            ...
            for (i = 1, plen = path.length; i < plen; i++) {
                context.lineTo(path[i][0] + 0.5 + this.offsetX, path[i][1] + 0.5 + this.offsetY);
            }
            if (lineColor !== undefined) {
                context.stroke();
            }
    ...
    let barHighlightMixin = {
        ...
        render: function (el, userValues) {
            this.init(el, userValues);
            let values = this.values, target = this.target, regionShapes = this.regionShapes, shapes, ids, i, j;
            for (i = values.length; i--;) {
