define([
    ...
    function orderbydatafiler(str, stc, edr, edc, index, asc) {
        ...
        data = m_sort.orderbydata(data, index - stc, asc);
        for (let r = str; r <= edr; r++) {
            for (let c = stc; c <= edc; c++) {
        ...
        if (Store.config['rowlen'] != null) {
            ...
            cfg = m_getRowlen.rowlenByRange(d, str, edr, cfg);
            allParam = {
                'cfg': cfg,
                'RowlChange': true
            };
        }
        m_refresh.jfrefreshgrid(d, [{
                'row': [
                    str,
                    edr
                ],
    ...
    function createFilter() {
        if (!m_protection.checkProtectionAuthorityNormal(Store.currentSheetIndex, 'filter')) {
            return;
        }
        ...
        if (Store.luckysheetfile[m_get.getSheetIndex(Store.currentSheetIndex)].isPivotTable) {
            return;
        }
        ...
        if (last['row'][0] == last['row'][1] && last['column'][0] == last['column'][1]) {
            ...
            for (let c = 0; c < Store.flowdata[curR].length; c++) {
                ...
                if (cell != null && !m_validate.isRealNull(cell.v)) {
                    if (st_c == null) {
                        st_c = c;
                    }
                } else if (st_c != null) {
            ...
            m_sheetMove.luckysheetMoveEndCell('down', 'range');
    ...
    function initialFilterHandler() {
        ...
        $('#luckysheet-cell-main').on('click', '.luckysheet-filter-options', function (e) {
            ...
            let $t = $(m_sheetMove.currentTarget), toffset = $t.offset(), $menu = $('#luckysheet-filter-menu'), winH = $(window).height(), winW = $(window).width();
            ...
            setTimeout(function () {
                ...
                for (let r = st_r + 1; r <= ed_r; r++) {
                    ...
                    if (cell != null && !m_validate.isRealNull(cell.v) && cell.ct != null && cell.ct.t == 'd') {
                        let v = m_format.update('YYYY-MM-DD', cell.v);
                        let y = v.split('-')[0];
                        let m = v.split('-')[1];
                        let d = v.split('-')[2];
                        if (!(y in dvmap)) {
                ...
                if (JSON.stringify(vmap).length > 2) {
                    ...
                    vmapKeys = m_sort.orderbydata1D(vmapKeys, true);
            ...
            m_util.showrightclickmenu($menu, toffset.left, toffset.top + 20);
            m_sheetMove.stopPropagation();
            return false;
        });
        ...
        $(document).off('click.orderbyColorConfirm').on('click.orderbyColorConfirm', '#luckysheet-filter-orderby-color-submenu #luckysheet-filter-orderby-color-confirm', function () {
            ...
            for (let r = st_r + 1; r <= ed_r; r++) {
                ...
                if (bg_filter && fc_filter) {
                    if (!(bg in bg_colorMap) && (!(fc in fc_colorMap) || cell == null || m_validate.isRealNull(cell.v))) {
                        rowhidden[r] = 0;
                    }
                } else if (bg_filter) {
                    ...
                    if (!(fc in fc_colorMap) || cell == null || m_validate.isRealNull(cell.v)) {
                        rowhidden[r] = 0;
                    }
                }
            }
            ...
            Store.config = cfg;
            Store.luckysheetfile[m_get.getSheetIndex(Store.currentSheetIndex)].config = Store.config;
        ...
        $('#luckysheet-filter-initial').click(function () {
            ...
            $('#luckysheet-filter-selected-sheet' + Store.currentSheetIndex + ', #luckysheet-filter-options-sheet' + Store.currentSheetIndex).remove();
            $('#luckysheet-filter-menu, #luckysheet-filter-submenu').hide();
            Store.luckysheetfile[m_get.getSheetIndex(Store.currentSheetIndex)].filter = null;
            Store.luckysheetfile[m_get.getSheetIndex(Store.currentSheetIndex)].filter_select = null;
            ...
            if ($('#luckysheet-filter-bycondition').next().is(':visible') && $('#luckysheet-filter-byvalue').next().is(':hidden') && $('#luckysheet-filter-selected span').data('value') != 'null') {
                ...
                for (let r = st_r + 1; r <= ed_r; r++) {
                    ...
                    if (value == 'cellnull') {
                        ...
                        let value1 = m_format.genarate(caljs['value1'])[2];
                        if (cell == null || m_validate.isRealNull(cell.v)) {
                            rowhidden[r] = 0;
                ...
                for (let r = st_r + 1; r <= ed_r; r++) {
                    ...
                    if (cell == null || m_validate.isRealNull(cell.v)) {
                        ...
                        let fmt = m_format.update('YYYY-MM-DD', cell.v);
