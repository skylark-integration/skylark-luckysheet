define([
    ...
    '../global/method',
    '../locale/locale'
], function (menuButton, formula, a, b, browser, dataVerificationCtrl, c, Store, method, locale) {
    'use strict';
    ...
    function selectHightlightShow(isRestore = false) {
        ...
        if (Store.luckysheet_select_save.length > 0) {
            ...
            if (Store.luckysheet_select_save.length == 1 && Store.luckysheet_select_save[0].row[0] == Store.luckysheet_select_save[0].row[1] && Store.luckysheet_select_save[0].column[0] == Store.luckysheet_select_save[0].column[1]) {
                a.dynamicArrayHightShow(Store.luckysheet_select_save[0].row[0], Store.luckysheet_select_save[0].column[0]);
            }
        }
        Store.luckysheetfile[c.getSheetIndex(Store.currentSheetIndex)].luckysheet_select_save = Store.luckysheet_select_save;
        ...
        if (Store.luckysheet_select_save_previous == null | Store.luckysheet_select_save_previous !== luckysheet_select_save_previous) {
            method.createHookFunction('rangeSelect', Store.luckysheetfile[c.getSheetIndex(Store.currentSheetIndex)], Store.luckysheet_select_save);
        }
    ...
    function selectTitlesShow(rangeArr, isRestore = false) {
        ...
        for (let i = 0; i < rowTitleRange.length; i++) {
            ...
            let row = b.rowLocationByIndex(r2)[1], row_pre = b.rowLocationByIndex(r1)[0];
        ...
        for (let j = 0; j < columnTitleRange.length; j++) {
            ...
            let col = b.colLocationByIndex(c2)[1], col_pre = b.colLocationByIndex(c1)[0];
    ...
    function collaborativeEditBox() {
        ...
        Store.cooperativeEdit.changeCollaborationSize.forEach(value => {
            if (value.i == Store.currentSheetIndex) {
                ...
                let range = Store.cooperativeEdit.merge_range;
                let change_left = all_width[value.v.column[0] - 1] - 1;
                let change_top = all_height[value.v.row[0] - 1] - 1;
    ...
    function selectHelpboxFill() {
        ...
        if (Store.config['merge'] != null && rf + '_' + cf in Store.config['merge']) {
            $('#luckysheet-helpbox-cell').text(c.getRangetxt(Store.currentSheetIndex, {
                column: [
                    cf,
                    cf
                ],
            ...
            $('#luckysheet-helpbox-cell').text(c.getRangetxt(Store.currentSheetIndex, range));
        }
    }
    return {
