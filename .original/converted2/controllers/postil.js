define([
    ...
    '../store',
    '../global/method'
], function (a, editor, formula, b, c, d, e, f, luckysheetFreezen, menuButton, g, server, Store, method) {
    'use strict';
    const luckysheetPostil = {
        ...
        init: function () {
            ...
            $('#luckysheet-postil-showBoxs').off('mousedown.resize').on('mousedown.resize', '.luckysheet-postil-show .luckysheet-postil-dialog-resize .luckysheet-postil-dialog-resize-item', function (event) {
                ...
                let mouse = a.mouseposition(event.pageX, event.pageY);
                ...
                d.setluckysheet_scroll_status(true);
        ...
        overshow: function (event) {
            ...
            let row_index = a.rowLocation(y)[2];
            let col_index = a.colLocation(x)[2];
        ...
        drawArrow: function (ctx, fromX, fromY, toX, toY, theta, headlen, width, color) {
            theta = f.getObjType(theta) == 'undefined' ? 30 : theta;
            headlen = f.getObjType(headlen) == 'undefined' ? 6 : headlen;
            width = f.getObjType(width) == 'undefined' ? 1 : width;
            color = f.getObjType(color) == 'undefined' ? '#000' : color;
        ...
        editPs: function (r, c) {
            ...
            $('#luckysheet-postil-show_' + r + '_' + c + ' .formulaInputFocus').focus();
            b.luckysheetRangeLast($('#luckysheet-postil-show_' + r + '_' + c + ' .formulaInputFocus').get(0));
            _this.init();
        },
        delPs: function (r, c) {
            if (!g.checkProtectionAuthorityNormal(Store.currentSheetIndex, 'editObjects')) {
        ...
        ref: function (data, rc) {
            ...
            Store.luckysheetfile[e.getSheetIndex(Store.currentSheetIndex)].data = Store.flowdata;
