define([
    ...
    const dataVerificationCtrl = {
        ...
        createDialog: function () {
            ...
            $('body').append(m_util.replaceHtml(g.modelHTML, {
        ...
        init: function () {
            ...
            $(document).off('click.dvSaveConfirm').on('click.dvSaveConfirm', '#luckysheet-dataVerification-dialog-confirm', function (e) {
                ...
                if (type == 'dropdown') {
                    ...
                    if (!m_datecontroll.isdatetime(value1)) {
                    ...
                    if (type2 == 'bw' || type2 == 'nb') {
                        ...
                        if (!m_datecontroll.isdatetime(value2)) {
                            tooltip.info('<i class="fa fa-exclamation-triangle"></i>', dvText.tooltipInfo6);
                            return;
                        }
                        if (m_datecontroll.diff(value1, value2) > 0) {
                ...
                for (let r = str; r <= edr; r++) {
                    for (let c = stc; c <= edc; c++) {
                        ...
                        if (type == 'checkbox') {
                            m_setdata.setcellvalue(r, c, d, item.value2);
                        }
                    }
                }
                if (type == 'checkbox') {
        ...
        dataAllocation: function () {
            ...
            let rangeTxt = i.getRangetxt(Store.currentSheetIndex, range, Store.currentSheetIndex);
        ...
        getTxtByRange: function (range) {
            if (range.length > 0) {
                ...
                for (let s = 0; s < range.length; s++) {
                    let r1 = range[s].row[0], r2 = range[s].row[1];
                    let c1 = range[s].column[0], c2 = range[s].column[1];
                    txt.push(i.getRangetxt(Store.currentSheetIndex, {
                        'row': [
                            r1,
                            r2
                        ],
        ...
        cellFocus: function (r, c, clickMode) {
            ...
            let cellValue = m_getdata.getcellvalue(r, c, null);
            if (m_validate.isRealNull(cellValue)) {
                return;
            }
        ...
        validateCellData: function (cellValue, item) {
            ...
            if (type == 'dropdown') {
                ...
                if (!m_validate.isRealNum(cellValue)) {
                    return false;
                }
                ...
                if (!m_datecontroll.isdatetime(cellValue)) {
                    return false;
                }
                if (type2 == 'bw' && (m_datecontroll.diff(cellValue, value1) < 0 || m_datecontroll.diff(cellValue, value2) > 0)) {
                    return false;
                }
                if (type2 == 'nb' && (m_datecontroll.diff(cellValue, value1) >= 0 && m_datecontroll.diff(cellValue, value2) <= 0)) {
                    return false;
                }
                if (type2 == 'eq' && m_datecontroll.diff(cellValue, value1) != 0) {
                    return false;
                }
                if (type2 == 'ne' && m_datecontroll.diff(cellValue, value1) == 0) {
                    return false;
                }
                if (type2 == 'bf' && m_datecontroll.diff(cellValue, value1) >= 0) {
                    return false;
                }
                if (type2 == 'nbf' && m_datecontroll.diff(cellValue, value1) < 0) {
                    return false;
                }
                if (type2 == 'af' && m_datecontroll.diff(cellValue, value1) <= 0) {
                    return false;
                }
                if (type2 == 'naf' && m_datecontroll.diff(cellValue, value1) > 0) {
                    return false;
                }
            } else if (type == 'validity') {
        ...
        getDropdownList: function (txt) {
            ...
            if (formula.iscelldata(txt)) {
                ...
                let d = Store.luckysheetfile[i.getSheetIndex(range.sheetIndex)].data;
        ...
        checkboxChange: function (r, c) {
            ...
            let d = editor.deepCopyFlowData(Store.flowdata);
            m_setdata.setcellvalue(r, c, d, value);
        ...
        ref: function (historyDataVerification, currentDataVerification, sheetIndex) {
            ...
            Store.luckysheetfile[i.getSheetIndex(sheetIndex)].dataVerification = currentDataVerification;
        ...
        refOfCheckbox: function (historyDataVerification, currentDataVerification, sheetIndex, d, range) {
            ...
            Store.luckysheetfile[i.getSheetIndex(sheetIndex)].data = Store.flowdata;
            if (server.allowUpdate) {
                server.saveParam('all', sheetIndex, currentDataVerification, { 'k': 'dataVerification' });
