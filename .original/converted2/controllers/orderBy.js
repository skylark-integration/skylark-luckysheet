define([
    ...
    '../store',
    '../locale/locale'
], function (a, b, c, d, e, f, cleargridelement, g, h, i, tooltip, editor, j, Store, locale) {
    'use strict';
    function orderByInitial() {
        const _locale = locale();
        $('#luckysheetorderbyasc, #luckysheetorderbyasc_t').mousedown(function (event) {
            cleargridelement(event);
            i.sortColumnSeletion(Store.orderbyindex, true);
            b.selectHightlightShow();
        });
        $('#luckysheetorderbydesc, #luckysheetorderbydesc_t').click(function (event) {
            cleargridelement(event);
            i.sortColumnSeletion(Store.orderbyindex, false);
            b.selectHightlightShow();
        });
        let luckysheet_sort_initial = true;
        $('#luckysheetorderby').click(function () {
            if (!c.checkProtectionAuthorityNormal(Store.currentSheetIndex, 'sort')) {
                return;
            }
            ...
            if (luckysheet_sort_initial) {
                ...
                $('body').append(d.replaceHtml(a.modelHTML, {
                ...
                $('#luckysheet-sort-dialog .luckysheet-sort-dialog-additem').click(function () {
                    ...
                    for (let c = c1; c <= c2; c++) {
                        if (t) {
                            let v = h.getcellvalue(r1, c, Store.flowdata, 'm');
                            if (v == null) {
                                v = locale_sort.columnOperation + (c - c1 + 1);
                            }
                            option += '<option value="' + c + '">' + v + '</option>';
                        } else {
                            option += '<option value="' + c + '">' + d.chatatABC(c) + '</option>';
                        }
                    }
                ...
                $('#luckysheet-sort-modal-confirm').click(function () {
                    ...
                    $($('#luckysheet-sort-dialog table tr').toArray().reverse()).each(function () {
                        ...
                        data = i.orderbydata([].concat(data), i, asc);
                    });
                    for (let r = str; r <= r2; r++) {
                        for (let c = c1; c <= c2; c++) {
                    ...
                    if (Store.config['rowlen'] != null) {
                        ...
                        cfg = e.rowlenByRange(d, str, r2, cfg);
                        allParam = {
                            'cfg': cfg,
                            'RowlChange': true
                        };
            ...
            $('#luckysheet-sort-dialog .luckysheet-modal-dialog-title-text').html(locale_sort.sortRangeTitle + '<span>' + d.chatatABC(c1) + (r1 + 1) + '</span>' + locale_sort.sortRangeTitleTo + '<span>' + d.chatatABC(c2) + (r2 + 1) + '</span>');
            ...
            if (r1 < r2) {
                setTimeout(function () {
                    ...
                    for (let i = c1; i <= c2; i++) {
                        let isdatatype_r1 = j.isdatatype(flowrowdata1[i]), isdatatype_r2 = j.isdatatype(flowrowdata2[i]);
