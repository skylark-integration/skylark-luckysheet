define([
    ...
    function luckysheet_compareWith() {
        ...
        let fp;
        if (m_util.getObjType(data_fp) == 'object' && data_fp.startCell != null) {
            if (sp == '&') {
        ...
        let tp;
        if (m_util.getObjType(data_tp) == 'object' && data_tp.startCell != null) {
            if (sp == '&') {
        ...
        if (m_validate.valueIsError(fp)) {
            return fp;
        }
        if (m_validate.valueIsError(tp)) {
            return tp;
        }
        if (m_util.getObjType(fp) == 'array' && m_util.getObjType(fp[0]) == 'array' && !func_methods.isDyadicArr(fp)) {
            return m_validate.error.v;
        }
        if (m_util.getObjType(tp) == 'array' && m_util.getObjType(tp[0]) == 'array' && !func_methods.isDyadicArr(tp)) {
            return m_validate.error.v;
        }
        if (sp == '<>') {
            sp = '!=';
        ...
        if (fp == null && tp == null) {
            ...
            return m_validate.error.d;
        }
        function booleanOperation(a, operator, b) {
            if (m_validate.isRealNum(a)) {
                a = parseFloat(a);
            }
            if (m_validate.isRealNum(b)) {
                b = parseFloat(b);
            }
            if (operator == '==') {
                if (a == b) {
        ...
        if (sp == '*') {
            if (m_util.getObjType(fp) == 'array' && m_util.getObjType(tp) == 'array') {
                let result = [];
                if (m_util.getObjType(fp[0]) == 'array' && m_util.getObjType(tp[0]) == 'array') {
                    if (fp.length == tp.length && fp[0].length == tp[0].length) {
                        for (let m = 0; m < fp.length; m++) {
                            ...
                            for (let n = 0; n < fp[m].length; n++) {
                                ...
                                let value;
                                if (m_validate.isRealNum(fp[m][n]) && m_validate.isRealNum(tp[m][n])) {
                                    value = luckysheet_calcADPMM(fp[m][n], sp, tp[m][n]);
                                } else {
                                    value = m_validate.error.v;
                                }
                                rowArr.push(value);
                            }
                            result.push(rowArr);
                        ...
                        for (let m = 0; m < rowlen; m++) {
                            ...
                            for (let n = 0; n < collen; n++) {
                                ...
                                for (let p = 0; p < fp[0].length; p++) {
                                    ...
                                    if (m_validate.isRealNum(fp[m][p]) && m_validate.isRealNum(tp[p][n])) {
                        ...
                        for (let m = 0; m < rowlen; m++) {
                            ...
                            for (let n = 0; n < collen; n++) {
                                ...
                                for (let p = 0; p < tp[0].length; p++) {
                                    ...
                                    if (m_validate.isRealNum(tp[m][p]) && m_validate.isRealNum(fp[p][n])) {
                        ...
                        return m_validate.error.na;
                    }
                } else if (m_util.getObjType(fp[0]) == 'array') {
                    if (fp[0].length == tp.length) {
                        for (let m = 0; m < fp.length; m++) {
                            ...
                            for (let n = 0; n < fp[m].length; n++) {
                                ...
                                let value;
                                if (m_validate.isRealNum(fp[m][n]) && m_validate.isRealNum(tp[n])) {
                        ...
                        for (let m = 0; m < rowlen; m++) {
                            ...
                            for (let n = 0; n < collen; n++) {
                                ...
                                tp[n] = booleanToNum(tp[n]);
                                let value;
                                if (m_validate.isRealNum(fp[m][0]) && m_validate.isRealNum(tp[n])) {
                ...
                } else if (m_util.getObjType(tp[0]) == 'array') {
                    if (tp[0].length == fp.length) {
                        for (let m = 0; m < tp.length; m++) {
                            ...
                            for (let n = 0; n < tp[m].length; n++) {
                                ...
                                tp[m][n] = booleanToNum(tp[m][n]);
                                let value;
                                if (m_validate.isRealNum(fp[n]) && m_validate.isRealNum(tp[m][n])) {
                        ...
                        for (let m = 0; m < rowlen; m++) {
                            ...
                            for (let n = 0; n < collen; n++) {
                                ...
                                let value;
                                if (m_validate.isRealNum(fp[n]) && m_validate.isRealNum(tp[m][0])) {
                    ...
                    for (let n = 0; n < fp.length; n++) {
                        ...
                        tp[n] = booleanToNum(tp[n]);
                        let value;
                        if (m_validate.isRealNum(fp[n]) && m_validate.isRealNum(tp[n])) {
                ...
                return result;
            } else if (m_util.getObjType(fp) == 'array') {
                tp = booleanToNum(tp);
                let result = [];
                if (m_util.getObjType(fp[0]) == 'array') {
                    for (let m = 0; m < fp.length; m++) {
                        let rowArr = [];
                        for (let n = 0; n < fp[m].length; n++) {
                            fp[m][n] = booleanToNum(fp[m][n]);
                            let value;
                            if (m_validate.isRealNum(fp[m][n]) && m_validate.isRealNum(tp)) {
                    ...
                    for (let n = 0; n < fp.length; n++) {
                        fp[n] = booleanToNum(fp[n]);
                        let value;
                        if (m_validate.isRealNum(fp[n]) && m_validate.isRealNum(tp)) {
                ...
                return result;
            } else if (m_util.getObjType(tp) == 'array') {
                fp = booleanToNum(fp);
                let result = [];
                if (m_util.getObjType(tp[0]) == 'array') {
                    for (let m = 0; m < tp.length; m++) {
                        let rowArr = [];
                        for (let n = 0; n < tp[m].length; n++) {
                            tp[m][n] = booleanToNum(tp[m][n]);
                            let value;
                            if (m_validate.isRealNum(fp) && m_validate.isRealNum(tp[m][n])) {
                    ...
                    for (let n = 0; n < tp.length; n++) {
                        tp[n] = booleanToNum(tp[n]);
                        let value;
                        if (m_validate.isRealNum(fp) && m_validate.isRealNum(tp[n])) {
                ...
                tp = booleanToNum(tp);
                let result;
                if (m_validate.isRealNum(fp) && m_validate.isRealNum(tp)) {
                    result = luckysheet_calcADPMM(fp, sp, tp);
                } else {
                    result = m_validate.error.v;
                }
                return result;
            }
        } else if (sp == '/') {
            if (m_util.getObjType(fp) == 'array' && m_util.getObjType(tp) == 'array') {
                ...
                if (m_util.getObjType(fp[0]) == 'array' && m_util.getObjType(tp[0]) == 'array') {
                    if (fp.length == tp.length && fp[0].length == tp[0].length) {
                        ...
                        let tp_inverse = m_matrix_methods.inverse(tp);
                        let rowlen = fp.length;
                        ...
                        for (let m = 0; m < rowlen; m++) {
                            ...
                            for (let n = 0; n < collen; n++) {
                                ...
                                for (let p = 0; p < fp[0].length; p++) {
                                    ...
                                    if (m_validate.isRealNum(fp[m][p]) && m_validate.isRealNum(tp_inverse[p][n])) {
    ...
    function luckysheet_getcelldata(txt) {
        ...
        let luckysheetfile = m_get.getluckysheetfile();
        let val = txt.split('!');
        ...
        if (val.length > 1) {
            ...
            let index = m_get.getSheetIndex(Store.calculateSheetIndex);
        ...
        if (rangetxt.indexOf(':') == -1) {
            ...
            let col = m_util.ABCatNum(rangetxt.replace(/[^A-Za-z]/g, ''));
            if (!isNaN(row) && !isNaN(col)) {
                let ret = m_getdata.getdatabyselectionD(sheetdata, {
                    'row': [
                        row,
                        row
                    ],
            ...
            col[0] = m_util.ABCatNum(rangetxt[0].replace(/[^A-Za-z]/g, ''));
            col[1] = m_util.ABCatNum(rangetxt[1].replace(/[^A-Za-z]/g, ''));
            if (isNaN(col[0])) {
                col[0] = 0;
            }
            if (isNaN(col[1])) {
    ...
    function luckysheet_parseData(value) {
        if (typeof value === 'object') {
            if (value == null) {
                ...
                let v = m_format.genarate(value[0]);
                return v[2];
            } else {
    ...
    function luckysheet_offset_check() {
        if (!(m_util.getObjType(arguments[0]) == 'object' && arguments[0].startCell != null)) {
            return formula.undefined.v;
        }
        var reference = arguments[0].startCell;
        var rows = func_methods.getFirstValue(arguments[1]);
        if (m_validate.valueIsError(rows)) {
            return rows;
        }
        if (!m_validate.isRealNum(rows)) {
            return formula.undefined.v;
        }
        rows = parseInt(rows);
        var cols = func_methods.getFirstValue(arguments[2]);
        if (m_validate.valueIsError(cols)) {
            return cols;
        }
        if (!m_validate.isRealNum(cols)) {
            return formula.undefined.v;
        }
        cols = parseInt(cols);
        ...
        if (arguments.length >= 4) {
            ...
            if (m_validate.valueIsError(height)) {
                return height;
            }
            if (!m_validate.isRealNum(height)) {
                return formula.undefined.v;
            }
        ...
        if (arguments.length == 5) {
            ...
            if (m_validate.valueIsError(width)) {
                return width;
            }
            if (!m_validate.isRealNum(width)) {
                return formula.undefined.v;
            }
        ...
        return m_get.getRangetxt(Store.calculateSheetIndex, {
            row: [
                cellRow0,
                cellRow1
            ],
