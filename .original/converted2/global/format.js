define([
    ...
    function genarate(value) {
        ...
        if (value.toString().substr(0, 1) === "'") {
            ...
            v = false;
        } else if (m_validate.valueIsError(value)) {
            m = value.toString();
            ct = {
                'fa': 'General',
                't': 'e'
        ...
        } else if (m_validate.isRealNum(value) && Math.abs(parseFloat(value)) > 0 && (Math.abs(parseFloat(value)) >= 100000000000 || Math.abs(parseFloat(value)) < 1e-9)) {
            ...
            if (index == value.toString().length - 1 && m_validate.isRealNum(value3)) {
                if (value2.indexOf('.') > -1) {
            ...
            if (value.toString().indexOf('.') == value.toString().lastIndexOf('.')) {
                ...
                if (value1.indexOf(',') > -1) {
                    ...
                    for (var i = 1; i < ThousandsArr.length; i++) {
                        if (!m_validate.isRealNum(ThousandsArr[i]) || ThousandsArr[i].length < 3) {
                            isThousands = false;
                            break;
                        }
                    }
                    ...
                    if (m_validate.isRealNum(value1) && m_validate.isRealNum(value2)) {
                        ct = {
                            'fa': '0.' + new Array(len + 1).join('0'),
                            't': 'n'
                        };
        ...
        } else if (m_validate.isRealNum(value)) {
            m = value.toString();
            ct = {
                'fa': 'General',
                't': 'n'
            ...
            v = parseFloat(value);
        } else if (m_datecontroll.isdatetime(value) && (value.toString().indexOf('.') > -1 || value.toString().indexOf(':') > -1 || value.toString().length < 16)) {
    ...
    function valueShowEs(r, c, d) {
        var value = m_getdata.getcellvalue(r, c, d, 'm');
        if (value == null) {
            value = m_getdata.getcellvalue(r, c, d, 'v');
        } else {
