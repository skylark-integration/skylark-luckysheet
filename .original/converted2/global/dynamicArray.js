define([
    ...
    function dynamicArrayCompute(dynamicArray) {
        ...
        if (m_util.getObjType(dynamicArray) == 'array') {
            for (let i = 0; i < dynamicArray.length; i++) {
                ...
                if (Store.flowdata[d_row][d_col] != null && Store.flowdata[d_row][d_col].f != null && Store.flowdata[d_row][d_col].f == d_f) {
                    ...
                    let d_collen = 1;
                    if (m_util.getObjType(d_data[0]) == 'array') {
    ...
    function dynamicArrayHightShow(r, c) {
        let dynamicArray = Store.luckysheetfile[m_get.getSheetIndex(Store.currentSheetIndex)]['dynamicArray'] == null ? [] : Store.luckysheetfile[m_get.getSheetIndex(Store.currentSheetIndex)]['dynamicArray'];
        ...
        if (r + '_' + c in dynamicArray_compute && dynamicArray_compute[r + '_' + c].v != '#SPILL!') {
            ...
            for (let i = 0; i < dynamicArray.length; i++) {
                if (dynamicArray[i].f == d_f) {
                    ...
                    if (m_util.getObjType(dynamicArray[i].data[0]) == 'array') {
