define([
    ...
    function rowlenByRange(d, r1, r2, cfg) {
        ...
        for (let r = r1; r <= r2; r++) {
            ...
            for (let c = 0; c < d[r].length; c++) {
                ...
                if (cell != null && (cell.v != null || e.isInlineStringCell(cell))) {
                    let cellWidth = m_location.colLocationByIndex(c)[1] - m_location.colLocationByIndex(c)[0] - 2;
    ...
    function getCellTextInfo(cell, ctx, option) {
        ...
        let space_width = option.space_width, space_height = option.space_height;
        if (space_width == null) {
            space_width = 2;
        }
        ...
        let horizonAlign = m_getdata.checkstatusByCell(cell, 'ht');
        let verticalAlign = m_getdata.checkstatusByCell(cell, 'vt');
        let tb = m_getdata.checkstatusByCell(cell, 'tb');
        let tr = m_getdata.checkstatusByCell(cell, 'tr');
        let rt = m_getdata.checkstatusByCell(cell, 'rt');
        ...
        if (e.isInlineStringCell(cell)) {
            ...
            fontset = m_util.luckysheetfontformat(cell);
            ctx.font = fontset;
            cancelLine = m_getdata.checkstatusByCell(cell, 'cl');
            underLine = m_getdata.checkstatusByCell(cell, 'un');
            fontSize = m_getdata.checkstatusByCell(cell, 'fs');
            ...
            if (tb == '2' || isInline) {
                ...
                if (isInline) {
                    while (i <= inlineStringArr.length) {
                        ...
                        let width = textWidth * Math.cos(rt * Math.PI / 180) + textHeight * Math.sin(rt * Math.PI / 180);
                        let height = textWidth * Math.sin(rt * Math.PI / 180) + textHeight * Math.cos(rt * Math.PI / 180);
                        let lastWord = shareCells[shareCells.length - 1];
                        if (lastWord.v == ' ' || d.checkWordByteLength(lastWord.v) == 2) {
                    ...
                    while (i <= value.length) {
                        ...
                        if (lastWord == ' ' || d.checkWordByteLength(lastWord) == 2) {
                ...
                rt = Math.abs(rt);
                let rtPI = rt * Math.PI / 180;
                let textWidthAll = textWidth * Math.cos(rtPI) + textHeight * Math.sin(rtPI);
                let textHeightAll = textWidth * Math.sin(rtPI) + textHeight * Math.cos(rtPI);
                if (rt != 0) {
                ...
                let left = space_width + textHeight * Math.sin(rtPI) * isRotateUp;
                if (horizonAlign == '0') {
                ...
                let top = cellHeight - space_height - height + measureText.actualBoundingBoxAscent * Math.cos(rtPI) + textWidth * Math.sin(rtPI) * isRotateUp;
                if (verticalAlign == '0') {
