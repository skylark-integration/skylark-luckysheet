define([
    ...
    './json',
    './method'
], function (a, b, c, d, sheetmanage, menuButton, server, luckysheetFreezen, e, dataVerificationCtrl, f, g, h, i, j, k, l, editor, tooltip, m, n, o, p, q, Store, locale, json, method) {
    'use strict';
    const luckysheetformula = {
        ...
        errorParamCheck: function (thisp, data, i) {
            ...
            if (type.indexOf('all') > -1) {
                ...
                if (type.indexOf('range') > -1 && (a.getObjType(data) == 'object' || a.getObjType(data) == 'array')) {
                    return [
                        true,
                        locale_formulaMore.tipSuccessText
                    ];
                }
                if (type.indexOf('number') > -1 && (g.isRealNum(data) || a.getObjType(data) == 'boolean')) {
                    return [
                        true,
                        locale_formulaMore.tipSuccessText
                    ];
                }
                if (type.indexOf('string') > -1 && a.getObjType(data) == 'string') {
                    return [
                        true,
                        locale_formulaMore.tipSuccessText
                    ];
                }
                if (type.indexOf('date') > -1 && h.isdatetime(data)) {
                    return [
                        true,
                        locale_formulaMore.tipSuccessText
                    ];
        ...
        getPureValueByData: function (data) {
            ...
            let output = [];
            if (a.getObjType(data) == 'array') {
                if (a.getObjType(data[0]) == 'array') {
                    for (let r = 0; r < data.length; r++) {
                        let row = [];
                        for (let c = 0; c < data[0].length; c++) {
                            let cell = data[r][c];
                            if (a.getObjType(cell) == 'object') {
                                row.push(cell.v);
                            } else {
                                row.push(cell);
                            }
        ...
        readCellDataToOneArray: function (rangeValue) {
            ...
            if (a.getObjType(rangeValue) != 'object') {
                return [rangeValue];
            }
            let dataformat = [];
            let data = [];
            ...
            } else if (rangeValue != null && !g.isRealNull(rangeValue.v)) {
                return [rangeValue.v];
            } else {
                return [];
            }
            ...
            } else if (a.getObjType(data) == 'object') {
                data = data.v;
                return [data];
            } else {
                if (/\{.*?\}/.test(data)) {
        ...
        getValueByFuncData: function (value, arg) {
            ...
            if (a.getObjType(value) == 'array') {
                if (arg == 'avg') {
                    ...
                    if (a.getObjType(value[0]) == 'object') {
                        return luckysheet.mask.getValueByFormat(value[0]);
                    } else {
                        return value[0];
                    }
                }
            } else if (a.getObjType(value) == 'object') {
        ...
        fucntionboxshow: function (r, c) {
            if (!e.checkProtectionCellHidden(r, c, Store.currentSheetIndex)) {
            ...
            if (d[r] != null && d[r][c] != null) {
                ...
                if (p.isInlineStringCell(cell)) {
                    value = j.getInlineStringNoStyle(r, c);
                } else if (cell.f != null) {
                    value = j.getcellvalue(r, c, d, 'f');
                } else {
                    value = l.valueShowEs(r, c, d);
                }
            }
        ...
        cellOffset: function (range, rows, cols, height, width) {
            ...
            let startCellCol = a.ABCatNum(startCell.replace(/[^A-Za-z]/g, ''));
            let row = [], col = [], offsetRange;
            row[0] = startCellRow + rows;
            col[0] = startCellCol + cols;
            row[1] = row[0] + height - 1;
            col[1] = col[0] + width - 1;
            col[0] = a.chatatABC(col[0]);
            col[1] = a.chatatABC(col[1]);
        ...
        parseDatetoNum: function (date) {
            ...
            if (typeof date == 'object' && typeof date.v == 'number') {
                date = date.v;
            } else if (h.isdatatype(date) == 'num') {
                date = parseFloat(date);
            } else if (h.isdatatype(date) == 'date') {
                date = l.genarate(date)[2];
            } else {
        ...
        acompareb: function (a, b) {
            ...
            let flag = false;
            if (g.isRealNum(b)) {
                flag = q.luckysheet_compareWith(a, '==', b);
                ...
                if (b.indexOf('*') != -1 || b.indexOf('?') != -1) {
                    ...
                    flag = q.luckysheet_compareWith(a, ope, num);
                } else {
                    flag = q.luckysheet_compareWith(a, '==', b);
                }
            }
        ...
        getcellrange: function (txt, formulaIndex) {
            ...
            let luckysheetfile = b.getluckysheetfile();
            if (txt.indexOf('!') > -1) {
            ...
            if (rangetxt.indexOf(':') == -1) {
                ...
                let col = a.ABCatNum(rangetxt.replace(/[^A-Za-z]/g, ''));
                ...
                col[0] = a.ABCatNum(rangetxt[0].replace(/[^A-Za-z]/g, ''));
                col[1] = a.ABCatNum(rangetxt[1].replace(/[^A-Za-z]/g, ''));
                if (isNaN(col[0])) {
                    col[0] = 0;
                }
                if (isNaN(col[1])) {
        ...
        createRangeHightlight: function () {
            ...
            $span.each(function () {
                ...
                $('#luckysheet-formula-functionrange').append(a.replaceHtml(_this.rangeHightlightHTML, { 'id': rangeindex }));
                ...
                if (cellrange == null) {
                    ...
                    $('#' + rangeid).data('range', cellrange).find('.luckysheet-copy').css({ 'background': d.luckyColor[rangeindex] }).end().find('.luckysheet-highlight').css({ 'background': d.luckyColor[rangeindex] }).end().find('.luckysheet-selection-copy-hc').css({ 'background': d.luckyColor[rangeindex] });
                    f.seletedHighlistByindex(rangeid, cellrange.row[0], cellrange.row[1], cellrange.column[0], cellrange.column[1]);
                }
            });
        ...
        searchFunctionHTML: function (list) {
            ...
            for (let i = 0; i < list.length; i++) {
                let item = list[i];
                if (i == list.length - 1) {
                    retHTML += a.replaceHtml(itemHTML, {
        ...
        helpFunction: function ($editer, funcname, paramIndex) {
            ...
            for (let i = 0; i < $func.p.length; i++) {
                ...
                fhcp += a.replaceHtml(helpformulaArg, {
                    'param': nameli,
            ...
            $('#luckysheet-formula-help-c .luckysheet-formula-help-title .luckysheet-arguments-parameter-holder').html(fht);
            $('#luckysheet-formula-help-c .luckysheet-arguments-help-formula .luckysheet-arguments-parameter-holder').html(ahf);
            $('#luckysheet-formula-help-c .luckysheet-formula-help-content-param').html(fhcp);
            if (paramIndex == null) {
        ...
        helpFunctionExe: function ($editer, currSelection) {
            ...
            if ($('#luckysheet-formula-help-c').length == 0) {
                $('body').after(a.replaceHtml(_this.helpHTML, {
        ...
        updatecell: function (r, c, value, isRefresh = true) {
            ...
            if (!e.checkProtectionLocked(r, c, Store.currentSheetIndex)) {
                return;
            }
            ...
            let isPrevInline = p.isInlineStringCell(curv);
            ...
            if (!value && !isCurInline && isPrevInline) {
                ...
                curv.ct.t = 'inlineStr';
                curv.ct.s = p.convertSpanToShareString($input.find('span'));
                if (isCopyVal) {
            ...
            if (!isCurInline) {
                if (g.isRealNull(value) && !isPrevInline) {
                    if (curv == null || g.isRealNull(curv.v) && curv.spl == null && curv.f == null) {
                        _this.cancelNormalSelected();
                        return;
                    }
                } else if (curv != null && curv.qp != 1) {
                    if (a.getObjType(curv) == 'object' && (value == curv.f || value == curv.v || value == curv.m)) {
                        _this.cancelNormalSelected();
                        return;
                    } else if (value == curv) {
                        _this.cancelNormalSelected();
                ...
                if (a.getObjType(value) == 'string' && value.slice(0, 1) == '=' && value.length > 1) {
                } else if (a.getObjType(curv) == 'object' && curv.ct != null && curv.ct.fa != null && curv.ct.fa != '@' && !g.isRealNull(value)) {
                    delete curv.m;
                    if (curv.f != null) {
                        delete curv.f;
                        delete curv.spl;
                ...
                if (!isCurInline) {
                    if (a.getObjType(value) == 'string' && value.slice(0, 1) == '=' && value.length > 1) {
                        ...
                        if (v.length == 4 && v[3].type == 'sparklines') {
                            ...
                            if (a.getObjType(curCalv) == 'array' && a.getObjType(curCalv[0]) != 'object') {
                                curv.v = curCalv[0];
                            } else {
                                curv.spl = v[3].data;
                            }
                        ...
                        if (a.getObjType(valueFunction) == 'string' && valueFunction.slice(0, 1) == '=' && valueFunction.length > 1) {
            ...
            k.setcellvalue(r, c, d, value);
            _this.cancelNormalSelected();
            let RowlChange = false;
            let cfg = $.extend(true, {}, b.getluckysheetfile()[b.getSheetIndex(Store.currentSheetIndex)]['config']);
            if (cfg['rowlen'] == null) {
                cfg['rowlen'] = {};
            }
            if (d[r][c].tb == '2' && d[r][c].v != null || p.isInlineStringCell(d[r][c])) {
                ...
                if (cfg['customHeight'] && cfg['customHeight'][r] == 1) {
                    ...
                    let cellWidth = m.colLocationByIndex(c)[1] - m.colLocationByIndex(c)[0] - 2;
                    let textInfo = i.getCellTextInfo(d[r][c], canvas, {
                        r: r,
                        c: c,
                        cellWidth: cellWidth
                    });
            ...
            if (isRefresh) {
                o.jfrefreshgrid(d, [{
                    ...
                    }], allParam, isRunExecFunction);
                _this.execFunctionGlobalData = null;
            } else {
                return {
                    data: d,
                    allParam: allParam
        ...
        setfreezonFuceExe: function (rangetxt) {
            ...
            let $row = '$', $col = '$';
            if (!isNaN(row) && !isNaN(col)) {
                return $col + a.chatatABC(col) + $row + row;
            } else if (!isNaN(row)) {
                return $row + row;
            } else if (!isNaN(col)) {
                return $col + a.chatatABC(col);
            } else {
                return rangetxt;
            }
        },
        ...
        updateparam: function (orient, txt, step) {
            ...
            if (rangetxt.indexOf(':') == -1) {
                ...
                if (!isNaN(row) && !isNaN(col)) {
                    return prefix + $col + a.chatatABC(col) + $row + row;
                } else if (!isNaN(row)) {
                    return prefix + $row + row;
                } else if (!isNaN(col)) {
                    return prefix + $col + a.chatatABC(col);
                } else {
                    return txt;
                }
            } else {
                ...
                if (isNaN(col[0]) && isNaN(col[1])) {
                    ...
                    return prefix + $col0 + a.chatatABC(col[0]) + ':' + $col1 + a.chatatABC(col[1]);
                } else {
                    return prefix + $col0 + a.chatatABC(col[0]) + $row0 + row[0] + ':' + $col1 + a.chatatABC(col[1]) + $row1 + row[1];
                }
            }
        },
        ...
        functionStrChange_range: function (txt, type, rc, orient, stindex, step) {
            ...
            if (rangetxt.indexOf(':') == -1) {
                ...
                c1 = c2 = a.ABCatNum(rangetxt.replace(/[^A-Za-z]/g, ''));
                let freezonFuc = _this.isfreezonFuc(rangetxt);
                ...
                c1 = a.ABCatNum(rangetxt[0].replace(/[^A-Za-z]/g, ''));
                c2 = a.ABCatNum(rangetxt[1].replace(/[^A-Za-z]/g, ''));
                if (c1 > c2) {
                    return txt;
                }
                let freezonFuc0 = _this.isfreezonFuc(rangetxt[0]);
            ...
            if (type == 'del') {
                ...
                if (r1 == r2 && c1 == c2) {
                    if (!isNaN(r1) && !isNaN(c1)) {
                        return prefix + $col0 + a.chatatABC(c1) + $row0 + (r1 + 1);
                    } else if (!isNaN(r1)) {
                        return prefix + $row0 + (r1 + 1);
                    } else if (!isNaN(c1)) {
                        return prefix + $col0 + a.chatatABC(c1);
                    } else {
                        return txt;
                    }
                } else {
                    if (isNaN(c1) && isNaN(c2)) {
                        ...
                        return prefix + $col0 + a.chatatABC(c1) + ':' + $col1 + a.chatatABC(c2);
                    } else {
                        return prefix + $col0 + a.chatatABC(c1) + $row0 + (r1 + 1) + ':' + $col1 + a.chatatABC(c2) + $row1 + (r2 + 1);
                    }
                }
            } else if (type == 'add') {
                if (rc == 'row') {
        ...
        rangeSetValue: function (selected, obj) {
            ...
            if (Store.config['merge'] != null && rf + '_' + cf in Store.config['merge']) {
                range = b.getRangetxt(Store.currentSheetIndex, {
                    column: [
                        cf,
                        cf
                    ],
                ...
                range = b.getRangetxt(Store.currentSheetIndex, selected, _this.rangetosheet);
            }
            let $editor;
            if (_this.rangestart || _this.rangedrag_column_start || _this.rangedrag_row_start) {
                if ($('#luckysheet-search-formula-parm').is(':visible') || $('#luckysheet-search-formula-parm-select').is(':visible')) {
                    ...
                    let txtdata = q.luckysheet_getcelldata(range).data;
                ...
                let function_str = '<span class="luckysheet-formula-functionrange-cell" rangeindex="' + _this.functionHTMLIndex + '" dir="auto" style="color:' + d.luckyColor[_this.functionHTMLIndex] + ';">' + range + '</span>';
        ...
        rangedrag: function (event) {
            let _this = this;
            let mouse = m.mouseposition(event.pageX, event.pageY);
            let x = mouse[0] + $('#luckysheet-cell-main').scrollLeft();
            let y = mouse[1] + $('#luckysheet-cell-main').scrollTop();
            let row_location = m.rowLocation(y), row = row_location[1], row_pre = row_location[0], row_index = row_location[2];
            let col_location = m.colLocation(x), col = col_location[1], col_pre = col_location[0], col_index = col_location[2];
            ...
            f.luckysheet_count_show(left, top, width, height, rowseleted, columnseleted);
            ...
            if ($('#luckysheet-ifFormulaGenerator-multiRange-dialog').is(':visible')) {
                let range = b.getRangetxt(Store.currentSheetIndex, {
                    'row': rowseleted,
        ...
        rangedrag_column: function (event) {
            ...
            _this.func_selectedrange['left_move'] = left;
            _this.func_selectedrange['width_move'] = width;
            f.luckysheet_count_show(left, row_pre, width, row - row_pre - 1, [
                0,
                row_index
            ], columnseleted);
            _this.rangeSetValue({
        ...
        rangedrag_row: function (event) {
            ...
            _this.func_selectedrange['top_move'] = top;
            _this.func_selectedrange['height_move'] = height;
            f.luckysheet_count_show(col_pre, top, col - col_pre - 1, height, rowseleted, [
                0,
                col_index
            ]);
            _this.rangeSetValue({
        ...
        rangeResizeDraging: function (event, luckysheetCurrentChartResizeObj, luckysheetCurrentChartResizeXy, luckysheetCurrentChartResize, luckysheetCurrentChartResizeWinW, luckysheetCurrentChartResizeWinH, ch_width, rh_height) {
            ...
            let rangetxt = b.getRangetxt(Store.currentSheetIndex, range, _this.rangetosheet);
            let $span = _this.rangeResizeTo.find("span[rangeindex='" + rangeindex + "']").html(rangetxt);
            n.luckysheetRangeLast(_this.rangeResizeTo[0]);
        ...
        getSelectedFromRange: function (obj) {
            ...
            let ret = {
                'row': [
                    m.rowLocation(row_st)[2],
                    m.rowLocation(row_ed)[2]
                ],
                'column': [
                    m.colLocation(col_st)[2],
                    m.colLocation(col_ed)[2]
                ]
            };
            return ret;
        },
        ...
        rangeMoveDraging: function (event, luckysheet_cell_selected_move_index, luckysheet_select_save, obj, sheetBarHeight, statisticBarHeight) {
            ...
            let row_s = luckysheet_select_save['row'][0] - row_index_original + m.rowLocation(y)[2], row_e = luckysheet_select_save['row'][1] - row_index_original + m.rowLocation(y)[2];
            let col_s = luckysheet_select_save['column'][0] - col_index_original + m.colLocation(x)[2], col_e = luckysheet_select_save['column'][1] - col_index_original + m.colLocation(x)[2];
            if (row_s < 0 || y < 0) {
                row_s = 0;
        ...
        setCaretPosition: function (textDom, children, pos) {
            try {
                ...
                n.luckysheetRangeLast(this.rangeResizeTo[0]);
            }
        },
        functionRange: function (obj, v, vp) {
            ...
            if (window.getSelection) {
                ...
                _this.functionRangeIndex.moveToElementText(obj);
                _this.functionRangeIndex.collapse(false);
        ...
        functionHTML: function (txt) {
            ...
            while (i < funcstack.length) {
                ...
                if (i == funcstack.length - 1) {
                    if (_this.iscelldata($.trim(str))) {
                        function_str += '<span class="luckysheet-formula-functionrange-cell" rangeindex="' + _this.functionHTMLIndex + '" dir="auto" style="color:' + d.luckyColor[_this.functionHTMLIndex] + ';">' + str + '</span>';
                        _this.functionHTMLIndex++;
        ...
        insertUpdateDynamicArray: function (dynamicArrayItem) {
            ...
            let luckysheetfile = b.getluckysheetfile();
            let file = luckysheetfile[b.getSheetIndex(index)];
        ...
        addFunctionGroup: function (r, c, func, index) {
            ...
            c.setluckysheetfile(luckysheetfile);
        },
        ...
        execFunctionGroup: function (origin_r, origin_c, value, index, data, isForce = false) {
            ...
            if (!window.undefined) {
                window.undefined = q.luckysheet_compareWith;
                window.undefined = q.luckysheet_getarraydata;
                window.undefined = q.luckysheet_getcelldata;
                window.undefined = q.luckysheet_parseData;
                window.undefined = q.luckysheet_getValue;
                window.undefined = q.luckysheet_indirect_check;
                window.undefined = q.luckysheet_indirect_check_return;
                window.undefined = q.luckysheet_offset_check;
                window.undefined = q.luckysheet_calcADPMM;
                window.undefined = q.luckysheet_getSpecialReference;
            }
            ...
            if (value != null) {
                ...
                k.setcellvalue(0, 0, cellCache, value);
            ...
            let sheets = b.getluckysheetfile();
            let sheetData = {};
            ...
            for (let i = 0; i < calcChains.length; i++) {
                ...
                let calc_funcStr = j.getcellFormula(formulaCell.r, formulaCell.c, formulaCell.index);
        ...
        execFunctionGroup1: function (origin_r, origin_c, value, index, data, isForce = false) {
            ...
            let luckysheetfile = b.getluckysheetfile();
            let dynamicArray_compute = luckysheetfile[b.getSheetIndex(Store.currentSheetIndex)]['dynamicArray_compute'] == null ? {} : luckysheetfile[b.getSheetIndex(Store.currentSheetIndex)]['dynamicArray_compute'];
            if (index == null) {
                index = Store.currentSheetIndex;
            }
            if (value != null) {
            ...
            if (_this.execFunctionExist == null) {
                for (let i = 0; i < group.length; i++) {
                    let item = group[i];
                    let file = luckysheetfile[b.getSheetIndex(item['index'])];
                    if (file == null) {
                        continue;
                    }
                    let cell = file.data[item.r][item.c];
                    let calc_funcStr = j.getcellFormula(item.r, item.c, item.index);
        ...
        functionDFS: function (u) {
            ...
            u.color = 'b';
            window.luckysheet_getcelldata_cache = null;
            let calc_funcStr = j.getcellFormula(u.r, u.c, u.index);
        ...
        groupValuesRefresh: function () {
            ...
            if (_this.groupValuesRefreshData.length > 0) {
                for (let i = 0; i < _this.groupValuesRefreshData.length; i++) {
                    ...
                    k.setcellvalue(item.r, item.c, data, updateValue);
        ...
        execfunction: function (txt, r, c, index, isrefresh, notInsertFunc) {
            ...
            if (a.getObjType(result) == 'object' && result.startCell != null) {
                if (a.getObjType(result.data) == 'array') {
                    result = _this.error.v;
                } else {
                    if (a.getObjType(result.data) == 'object' && !g.isRealNull(result.data.v)) {
                        result = result.data.v;
                    } else if (!g.isRealNull(result.data)) {
            ...
            let dynamicArrayItem = null;
            if (a.getObjType(result) == 'array') {
                let isErr = false;
                if (a.getObjType(result[0]) != 'array' && result.length == 2) {
                    isErr = g.valueIsError(result[0]);
                }
                if (!isErr) {
                    if (a.getObjType(result[0]) == 'array' && result.length == 1 && result[0].length == 1) {
                        result = result[0][0];
                    } else {
                        dynamicArrayItem = {
                            'r': r,
