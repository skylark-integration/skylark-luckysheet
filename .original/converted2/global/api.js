define([
    ...
    function getCellValue(row, column, options = {}) {
        if (!m_validate.isRealNum(row) || !m_validate.isRealNum(column)) {
            return tooltip.info('Arguments row or column cannot be null or undefined.', '');
        }
        let curSheetOrder = m_get.getSheetIndex(Store.currentSheetIndex);
        ...
        let return_v;
        if (m_util.getObjType(cellData) == 'object') {
    ...
    function setCellValue(row, column, value, options = {}) {
        ...
        let {order = m_get.getSheetIndex(Store.currentSheetIndex), isRefresh = true, success} = { ...options };
        ...
        if (value == null || value.toString().length == 0) {
            ...
            m_setdata.setcellvalue(row, column, data, value);
        } else if (value instanceof Object) {
            let curv = {};
            let cell = data[row][column];
            if (m_validate.isRealNull(cell)) {
                cell = {};
            }
            if (value.f != null && value.v == null) {
                ...
                data = luckysheetformula.updatecell(row, column, curv, false).data;
            } else {
                if (value.ct != null) {
                    curv.ct = value.ct;
                }
                ...
                formula.delFunctionGroup(row, column);
                m_setdata.setcellvalue(row, column, data, curv);
            }
            for (let attr in value) {
                let v = value[attr];
                if (attr in formatList) {
                    menuButton.updateFormatCell(data, attr, v, row, row, column, column);
                } else {
                    cell[attr] = v;
                }
            }
    ...
    function clearCell(row, column, options = {}) {
        ...
        if (order === curSheetOrder) {
            h.jfrefreshgrid(targetSheetData, [{
                    row: [
                        row,
                        row
                    ],
    ...
    function deleteCell(move, row, column, options = {}) {
        ...
        let curSheetOrder = m_get.getSheetIndex(Store.currentSheetIndex);
        let {order = curSheetOrder, success} = { ...options };
        let moveType = 'move' + move.replace(move[0], move[0].toUpperCase());
        let sheetIndex;
        if (order) {
        ...
        m_extend.luckysheetDeleteCell(moveType, row, row, column, column, sheetIndex);
        if (success && typeof success === 'function') {
            success();
        }
    }
    ...
    function replace(content, replaceContent, options = {}) {
        ...
        if (file.index == Store.currentSheetIndex) {
            h.jfrefreshgrid(fileData, undefined, undefined, true, false);
        }
        h.luckysheetrefreshgrid();
    ...
    function enterEditMode(options = {}) {
        if ($('#luckysheet-conditionformat-dialog').is(':visible')) {
            ...
            s.luckysheetupdateCell(row_index, col_index, Store.flowdata);
        }
        if (options.success && typeof options.success === 'function') {
            options.success();
        }
    ...
    function frozenFirstRow(order) {
        ...
        if (!order || order == m_get.getSheetIndex(Store.currentSheetIndex)) {
            let scrollTop = $('#luckysheet-cell-main').scrollTop();
            let row_st = n.luckysheet_searcharray(Store.visibledatarow, scrollTop);
            if (row_st == -1) {
                row_st = 0;
            }
    ...
    function frozenFirstColumn(order) {
        ...
        if (!order || order == m_get.getSheetIndex(Store.currentSheetIndex)) {
            ...
            let col_st = n.luckysheet_searcharray(Store.visibledatacolumn, scrollLeft);
            if (col_st == -1) {
                col_st = 0;
            }
    ...
    function setBothFrozen(isRange, options = {}) {
        let curSheetOrder = m_get.getSheetIndex(Store.currentSheetIndex);
        let {range, order = curSheetOrder, success} = { ...options };
        let isCurrentSheet = !order || order == m_get.getSheetIndex(Store.currentSheetIndex);
        const locale_frozen = locale().freezen;
        cancelFrozen(order);
        if (!isRange) {
    ...
    function insertRowOrColumn(type, index = 0, options = {}) {
        ...
        if (!m_validate.isRealNum(number)) {
            if (m_validate.isEditMode()) {
        ...
        m_extend.luckysheetextendtable(type, index, number, 'lefttop', sheetIndex);
        if (success && typeof success === 'function') {
            success();
        }
    }
    ...
    function deleteRowOrColumn(type, startIndex, endIndex, options = {}) {
        if (!m_validate.isRealNum(startIndex) || !m_validate.isRealNum(endIndex)) {
        ...
        m_extend.luckysheetdeletetable(type, startIndex, endIndex, sheetIndex);
        if (success && typeof success === 'function') {
            success();
        }
    }
    ...
    function setRowHeight(rowInfo, options = {}) {
        if (m_util.getObjType(rowInfo) != 'object') {
            return tooltip.info('The rowInfo parameter is invalid.', '');
        }
        let {order = m_get.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
    ...
    function setColumnWidth(columnInfo, options = {}) {
        if (m_util.getObjType(columnInfo) != 'object') {
    ...
    function getRowHeight(rowInfo, options = {}) {
        if (m_util.getObjType(rowInfo) != 'array' || rowInfo.length == 0) {
            return tooltip.info('The rowInfo parameter is invalid.', '');
        }
        let {order = m_get.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
    ...
    function getColumnWidth(columnInfo, options = {}) {
        if (m_util.getObjType(columnInfo) != 'array' || columnInfo.length == 0) {
            return tooltip.info('The columnInfo parameter is invalid.', '');
        }
        let {order = m_get.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
    ...
    function getRangeAxis() {
        ...
        rangeArr.forEach(ele => {
            let axisText = m_get.getRangetxt(sheetIndex, {
                column: ele.column,
                row: ele.row
            });
    ...
    function getRangeValue(options = {}) {
        let curOrder = m_get.getSheetIndex(Store.currentSheetIndex);
        let {range, order = curOrder} = { ...options };
        let file = Store.luckysheetfile[order];
        if (!range || typeof range === 'object') {
            return m_getdata.getdatabyselection(range, file.index);
        } else if (typeof range === 'string') {
            if (formula.iscelldata(range)) {
                return m_getdata.getdatabyselection(formula.getcellrange(range), file.index);
            } else {
    ...
    function getRangeHtml(options = {}) {
        let {range = Store.luckysheet_select_save, order = m_get.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        if (m_util.getObjType(range) == 'string') {
            if (!formula.iscelldata(range)) {
        ...
        } else if (m_util.getObjType(range) == 'object') {
        ...
        if (m_util.getObjType(range) != 'array') {
            return tooltip.info('The range parameter is invalid.', '');
        }
        let file = Store.luckysheetfile[order];
        if (file == null) {
        ...
        if (cfg['merge'] != null) {
            ...
            for (let s = 0; s < range.length; s++) {
                ...
                has_PartMC = m_validate.hasPartMC(cfg, r1, r2, c1, c2);
                if (has_PartMC) {
                    break;
                }
            }
        ...
        if (range.length > 1) {
            ...
            if (!isSameRow && !isSameCol || o.selectIsOverlap(range)) {
                return tooltip.info('Cannot perform this operation on multiple selection areas, please select a single area', '');
            }
        }
        ...
        if (cfg['borderInfo'] && cfg['borderInfo'].length > 0) {
            borderInfoCompute = l.getBorderInfoCompute(file.index);
        }
        let d = file.data;
            ...
            for (let j = 0; j < colIndexArr.length; j++) {
                ...
                if (d[r] != null && d[r][c] != null) {
                    ...
                    if (d[r][c].ct != null && d[r][c].ct.fa != null && d[r][c].ct.fa.match(reg)) {
                        c_value = m_getdata.getcellvalue(r, c, d);
                    } else {
                        c_value = m_getdata.getcellvalue(r, c, d, 'm');
                    }
                    style += menuButton.getStyleByCell(d, r, c);
                    if (a.getObjType(d[r][c]) == 'object' && 'mc' in d[r][c]) {
                        if ('rs' in d[r][c]['mc']) {
    ...
    function getRangeArray(dimensional, options = {}) {
        ...
        let {range = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1], order = m_get.getSheetIndex(Store.currentSheetIndex)} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
        if (a.getObjType(range) == 'string') {
            if (!formula.iscelldata(range)) {
                return tooltip.info('The range parameter is invalid.', '');
            }
            range = formula.getcellrange(range);
        }
        if (a.getObjType(range) != 'object' || range.row == null || range.column == null) {
            return tooltip.info('The range parameter is invalid.', '');
        }
        ...
        let cfg = $.extend(true, {}, file.config);
        if (cfg['merge'] != null) {
            let has_PartMC = m_validate.hasPartMC(cfg, r1, r2, c1, c2);
            if (has_PartMC) {
                return tooltip.info('Cannot perform this operation on partially merged cells', '');
            }
        }
    ...
    function getRangeJson(isFirstRowTitle, options = {}) {
        ...
        if (config['merge'] != null) {
            ...
            has_PartMC = m_validate.hasPartMC(config, r1, r2, c1, c2);
            if (has_PartMC) {
                if (m_validate.isEditMode()) {
        ...
        let getdata = m_getdata.getdatabyselection(range, file.index);
        let arr = [];
        if (getdata.length === 0) {
            return;
        }
        if (isFirstRowTitle) {
            if (getdata.length === 1) {
                ...
                for (let i = 0; i < getdata[0].length; i++) {
                    obj[m_getdata.getcellvalue(0, i, getdata)] = '';
                }
                arr.push(obj);
            } else {
                for (let r = 1; r < getdata.length; r++) {
                    ...
                    for (let c = 0; c < getdata[0].length; c++) {
                        if (m_getdata.getcellvalue(0, c, getdata) == undefined) {
                            obj[''] = m_getdata.getcellvalue(r, c, getdata);
                        } else {
                            obj[m_getdata.getcellvalue(0, c, getdata)] = m_getdata.getcellvalue(r, c, getdata);
                        }
                    }
                    arr.push(obj);
                }
            ...
            for (let r = 0; r < getdata.length; r++) {
                let obj = {};
                for (let c = 0; c < getdata[0].length; c++) {
                    obj[a.chatatABC(c + st)] = m_getdata.getcellvalue(r, c, getdata);
                }
                arr.push(obj);
            }
        }
    ...
    function getRangeDiagonal(type, options = {}) {
        ...
        let getdata = m_getdata.getdatabyselection(range, order);
        let arr = [];
        if (getdata.length === 0) {
            return;
        }
    ...
    function setRangeShow(range, options = {}) {
        ...
        if (m_util.getObjType(range) == 'array') {
            for (let i = 0; i < range.length; i++) {
                if (m_util.getObjType(range[i]) === 'string') {
        ...
        let {show = true, order = m_get.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
    ...
    function setSingleRangeFormat(attr, value, options = {}) {
        ...
        if (m_util.getObjType(range) != 'object' || range.row == null || range.column == null) {
            tooltip.info('The range parameter is invalid.', '');
            return 'error';
        }
    ...
    function setRangeFilter(type, options = {}) {
        ...
        let {range = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1], order = m_get.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
        ...
        if (type == 'open') {
            ...
            if (file.index == Store.currentSheetIndex) {
                q.createFilterOptions(range, file.filter);
            }
            return {
                'row': range.row,
                'column': range.column
    ...
    function setRangeMerge(type, options = {}) {
        ...
        let curSheetOrder = m_get.getSheetIndex(Store.currentSheetIndex), curRange = Store.luckysheet_select_save;
        let {range = curRange, order = curSheetOrder, success} = { ...options };
        ...
        for (let i = 0; i < range.length; i++) {
            ...
            for (let r = r1; r <= r2; r++) {
                for (let c = c1; c <= c2; c++) {
                    let cell = data[r][c];
                    if (m_util.getObjType(cell) == 'object' && 'mc' in cell) {
                        isHasMc = true;
                        break;
                    }
                }
        ...
        if (isHasMc) {
            ...
            for (let i = 0; i < range.length; i++) {
                ...
                if (type == 'all') {
                    ...
                    for (let r = r1; r <= r2; r++) {
                        for (let c = c1; c <= c2; c++) {
                            let cell = data[r][c];
                            if (cell != null && (!m_validate.isRealNull(cell.v) || cell.f != null) && !isfirst) {
                                fv = $.extend(true, {}, cell);
                                isfirst = true;
                            }
                            data[r][c] = {
            ...
            if (order == curSheetOrder) {
                ...
                h.jfrefreshgrid(data, range, { 'cfg': cfg });
    ...
    function cancelRangeMerge(options = {}) {
        let curRange = Store.luckysheet_select_save, curSheetOrder = m_get.getSheetIndex(Store.currentSheetIndex);
        let {range = curRange, order = curSheetOrder, success} = { ...options };
        let file = Store.luckysheetfile[order], cfg = $.extend(true, {}, file.config), data = $.extend(true, [], file.data);
        if (data.length == 0) {
            data = $.extend(true, [], sheetmanage.buildGridData(file));
    ...
    function setRangeSort(type, options = {}) {
        ...
        let curSheetOrder = m_get.getSheetIndex(Store.currentSheetIndex), curRange = Store.luckysheet_select_save[0];
        let {range = curRange, order = curSheetOrder, success} = { ...options };
        ...
        data = m_sort.orderbydata([].concat(data), 0, type === 'asc');
        for (let r = r1; r <= r2; r++) {
            for (let c = c1; c <= c2; c++) {
        ...
        let allParam = {};
        if (cfg['rowlen'] != null) {
            cfg = m_getRowlen.rowlenByRange(fileData, r1, r2, cfg);
            allParam = {
                'cfg': cfg,
                'RowlChange': true
            };
    ...
    function setRangeSortMulti(hasTitle, sort, options = {}) {
        ...
        sort.forEach(sortItem => {
            let i = sortItem.i;
            i -= c1;
            data = m_sort.orderbydata([].concat(data), i, sortItem.sort === 'asc');
        });
        for (let r = str; r <= r2; r++) {
            for (let c = c1; c <= c2; c++) {
        ...
        let allParam = {};
        if (cfg['rowlen'] != null) {
            cfg = m_getRowlen.rowlenByRange(fileData, str, r2, cfg);
            allParam = {
                'cfg': cfg,
                'RowlChange': true
            };
    ...
    function setRangeConditionalFormatDefault(conditionName, conditionValue, options = {}) {
        ...
        if (m_util.getObjType(conditionValue) != 'array' || conditionValue.length == 0) {
            return tooltip.info('The conditionValue parameter is invalid.', '');
        }
        let {format = {
                'textColor': '#000000',
                'cellColor': '#ff0000'
            }, cellrange = Store.luckysheet_select_save, order = m_get.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        let data = file.data;
        if (data == null || data.length == 0) {
            data = sheetmanage.buildGridData(file);
        ...
        if (conditionName == 'betweenness') {
            ...
            if (rangeArr1.length > 1) {
                ...
                if (r1 == r2 && c1 == c2) {
                    v1 = m_getdata.getcellvalue(r1, c1, data);
                    conditionRange.push({
                        'row': rangeArr1[0].row,
            ...
            if (rangeArr2.length > 1) {
                ...
                if (r1 == r2 && c1 == c2) {
                    v2 = m_getdata.getcellvalue(r1, c1, data);
                    conditionRange.push({
                        'row': rangeArr2[0].row,
            ...
            if (rangeArr.length > 1) {
                ...
                if (r1 == r2 && c1 == c2) {
                    v = m_getdata.getcellvalue(r1, c1, data);
                    conditionRange.push({
                        'row': rangeArr[0].row,
            ...
            let v1 = conditionValue[0];
            let v2 = conditionValue[1];
            if (!k.isdatetime(v1) || !k.isdatetime(v2)) {
                return tooltip.info('The conditionValue parameter is invalid.', '');
            }
            let v;
            if (k.diff(v1, v2) > 0) {
        ...
        if (m_util.getObjType(cellrange) == 'string') {
            cellrange = conditionformat.getRangeByTxt(cellrange);
        } else if (m_util.getObjType(cellrange) == 'object') {
            cellrange = [cellrange];
        }
        if (m_util.getObjType(cellrange) != 'array') {
    ...
    function setRangeConditionalFormat(type, options = {}) {
        ...
        let {format, cellrange = Store.luckysheet_select_save, order = m_get.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('Incorrect worksheet index', '');
        }
        if (type == 'dataBar') {
            ...
            if (m_util.getObjType(format) != 'array' || format.length < 1 || format.length > 2) {
                return tooltip.info('The format parameter is invalid.', '');
            }
            ...
            if (m_util.getObjType(format) != 'array' || format.length < 2 || format.length > 3) {
                return tooltip.info('The format parameter is invalid.', '');
            }
        } else if (type == 'icons') {
            if (format == null) {
            ...
            if (m_util.getObjType(format) != 'string' || !formatValues.includes(format)) {
                return tooltip.info('The format parameter is invalid.', '');
            }
            switch (format) {
    ...
    function deleteRangeConditionalFormat(itemIndex, options = {}) {
        if (!m_validate.isRealNum(itemIndex)) {
    ...
    function deleteRange(move, options = {}) {
        ...
        if (move == 'left') {
            m_extend.luckysheetDeleteCell('moveLeft', str, edr, stc, edc, order);
        } else if (move == 'up') {
            m_extend.luckysheetDeleteCell('moveUp', str, edr, stc, edc, order);
        }
        if (success && typeof success === 'function') {
            success();
        }
    ...
    function matrixOperation(type, options = {}) {
        ...
        let getdata = m_getdata.getdatabyselection(range);
        let arr = [];
        if (getdata.length === 0) {
            return;
        }
    ...
    function setSheetAdd(options = {}) {
        ...
        $('#luckysheet-sheet-container-c').append(m_util.replaceHtml(p.sheetHTML, {
            'index': index,
            'active': '',
            'name': sheetname,
            'style': '',
    ...
    function setSheetCopy(options = {}) {
        let {targetOrder, order = m_get.getSheetIndex(Store.currentSheetIndex), success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
        ...
        if (!m_validate.isRealNum(targetOrder)) {
    ...
    function setSheetActive(order, options = {}) {
        if (order == null || !m_validate.isRealNum(order) || Store.luckysheetfile[order] == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
        let file = Store.luckysheetfile[order];
        let {success} = { ...options };
    ...
    function setSheetName(name, options = {}) {
        if (m_util.getObjType(name) != 'string' || name.toString().length == 0) {
    ...
    function setSheetColor(color, options = {}) {
        if (m_util.getObjType(color) != 'string' || color.toString().length == 0) {
    ...
    function setSheetMove(type, options = {}) {
        if (type != 'left' && type != 'right' && !m_validate.isRealNum(type)) {
            return tooltip.info('Type parameter not available', '');
        }
        if (m_validate.isRealNum(type)) {
            type = parseInt(type);
        }
        let curOrder = m_get.getSheetIndex(Store.currentSheetIndex);
    ...
    function setSheetZoom(zoom, options = {}) {
        if (!m_validate.isRealNum(zoom) || zoom < 0.1 || zoom > 4) {
    ...
    function showGridLines(options = {}) {
        ...
        if (file.index == Store.currentSheetIndex) {
            ...
            setTimeout(function () {
                m_refresh.luckysheetrefreshgrid();
            }, 1);
        }
        setTimeout(() => {
            if (success && typeof success === 'function') {
    ...
    function scroll(options = {}) {
        ...
        if (scrollLeft != null) {
            if (!m_validate.isRealNum(scrollLeft)) {
                return tooltip.info('The scrollLeft parameter is invalid.', '');
            }
            $('#luckysheet-scrollbar-x').scrollLeft(scrollLeft);
        } else if (targetColumn != null) {
            if (!m_validate.isRealNum(targetColumn)) {
    ...
    function getScreenshot(options = {}) {
        ...
        let str = range.row[0], edr = range.row[1], stc = range.column[0], edc = range.column[1];
        let has_PartMC = m_validate.hasPartMC(Store.config, str, edr, stc, edc);
        if (has_PartMC) {
            return tooltip.info('Cannot perform this operation on partially merged cells', '');
        }
        ...
        m.luckysheetDrawMain(scrollWidth, scrollHeight, ch_width, rh_height, 1, 1, null, null, newCanvas);
        ...
        ctx_newCanvas.strokeStyle = p.luckysheetdefaultstyle.strokeStyle;
    ...
    function getSheetData(options = {}) {
        let {
            order = m_get.getSheetIndex(Store.currentSheetIndex)
        } = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
    ...
    function setConfig(cfg, options = {}) {
        if (m_util.getObjType(cfg) != 'object') {
        ...
        if (file.index == Store.currentSheetIndex) {
            ...
            if ('rowhidden' in cfg || 'colhidden' in cfg || 'rowlen' in cfg || 'columnlen' in cfg) {
                m_refresh.jfrefreshgrid_rhcw(Store.flowdata.length, Store.flowdata[0].length);
            }
            setTimeout(function () {
                m_refresh.luckysheetrefreshgrid();
            }, 1);
    ...
    function getLuckysheetfile() {
        return m_get.getluckysheetfile();
    }
    function setDataVerification(optionItem, options = {}) {
        if (m_util.getObjType(optionItem) != 'object') {
        ...
        if (type == 'dropdown') {
            ...
            if (!k.isdatetime(value1)) {
                tooltip.info('<i class="fa fa-exclamation-triangle"></i>', dvText.tooltipInfo6);
                return;
            }
            if (type2 == 'bw' || type2 == 'nb') {
                if (!k.isdatetime(value2)) {
                    tooltip.info('<i class="fa fa-exclamation-triangle"></i>', dvText.tooltipInfo6);
                    return;
                }
                if (k.diff(value1, value2) > 0) {
        ...
        for (let r = str; r <= edr; r++) {
            for (let c = stc; c <= edc; c++) {
                ...
                if (type == 'checkbox') {
                    item.checked ? m_setdata.setcellvalue(r, c, data, item.value1) : m_setdata.setcellvalue(r, c, data, item.value2);
                }
            }
        }
        if (file.index == Store.currentSheetIndex) {
    ...
    function insertImage(src, options = {}) {
        let {order = m_get.getSheetIndex(Store.currentSheetIndex), rowIndex, colIndex, success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
    ...
    function deleteImage(options = {}) {
        let {order = m_get.getSheetIndex(Store.currentSheetIndex), idList = 'all', success} = { ...options };
        let file = Store.luckysheetfile[order];
        if (file == null) {
            return tooltip.info('The order parameter is invalid.', '');
        }
        ...
        if (idList != 'all' && m_util.getObjType(idList) != 'array') {
            return tooltip.info('The idList parameter is invalid.', '');
        }
        if (m_util.getObjType(idList) == 'array') {
            idList.forEach(item => {
                delete images[item];
            });
        } else {
    ...
    function toJson() {
        ...
        m_get.getluckysheetfile().forEach((file, index) => {
            if (file.data == undefined) {
                return;
            }
            toJsonOptions.data[index].row = m_util.getObjType(file.data) === 'array' ? file.data.length : 0;
            toJsonOptions.data[index].column = m_util.getObjType(file.data[0]) === 'array' ? file.data[0].length : 0;
        });
    ...
    function getTxtByRange(range = Store.luckysheet_select_save) {
        if (m_util.getObjType(range) === 'object') {
            range = [range];
        }
