define([
    ...
    function ceateABC(index) {
        let wordlen = m_constant.columeHeader_word.length;
        if (index < wordlen) {
            return m_constant.columeHeader_word;
        } else {
            let relist = [];
            let i = 2, n = 0;
            ...
            for (let x = 0; x < n; x++) {
                if (x == 0) {
                    relist = relist.concat(m_constant.columeHeader_word);
                } else {
    ...
    function createABCdim(x, count) {
        let chwl = m_constant.columeHeader_word.length;
        if (x == 1) {
            let ret = [];
            let c = 0, con = true;
            for (let i = 0; i < chwl; i++) {
                let b = m_constant.columeHeader_word[i];
                for (let n = 0; n < chwl; n++) {
                    let bq = b + m_constant.columeHeader_word[n];
                    ret.push(bq);
                    c++;
                    if (c > index) {
                        return ret;
            ...
            let c = 0, con = true;
            for (let i = 0; i < chwl; i++) {
                let bb = m_constant.columeHeader_word[i];
                for (let w = 0; w < chwl; w++) {
                    let aa = m_constant.columeHeader_word[w];
                    for (let n = 0; n < chwl; n++) {
                        let bqa = bb + aa + m_constant.columeHeader_word[n];
                        ret.push(bqa);
                        c++;
                        if (c > index) {
                            return ret;
    ...
    function getByteLen(val, subLen) {
        ...
        for (let i = 0; i < val.length; i++) {
            ...
            if (m_validate.isRealNum(subLen) && len === ~~subLen) {
    ...
    function luckysheetfontformat(format) {
        ...
        if (getObjType(format) == 'object') {
            ...
            if (!format.ff) {
                ...
                if (m_datecontroll.isdatatypemulti(format.ff)['num']) {
            ...
            return m_constant.luckysheetdefaultFont();
        }
    }
    ...
    function numFormat(num, type) {
        if (num == null || isNaN(parseFloat(num)) || m_validate.hasChinaword(num) || num == -Infinity || num == Infinity) {
            return null;
        }
        ...
        if (type == null || type == 'auto') {
            ...
            if (m_datecontroll.isdatatype(type) == 'num') {
    ...
    function numfloatlen(n) {
        if (n != null && !isNaN(parseFloat(n)) && !m_validate.hasChinaword(n)) {
